"""
P90 Pure Cascade — Multi-Pair Backtest (No DMR)
=================================================
Uses p90_engine.py (pure P90, no DMR overlay).
Proper calibration: hour-based P90 thresholds + per-asset tier config.

P90 Logic:
  - Entry: Immediate on P90 candle close (body >= threshold, close outside Asian Range)
  - INITIAL: First P90 of session, SL = 0.80x body
  - CASCADE: Same-direction P90 within 120min, SL = 1.68x body
  - TP1: -25% Asian Range, TP2: -50% Asian Range
  - EWS: Opposite P90 at target = exit signal
  - Session: 3AM-12PM EST

Reference: Pure P90 baseline achieved 78.7% WR on EURUSD (1,038 trades).
"""
import sys, json, csv
...
def run_p90_backtest(symbol: str, bars: list, config: dict) -> dict:
    """Run pure P90 backtest (no DMR) for a single pair."""
    engine = P90Engine(
...
def main():
    ...
    # Summary
    print("\n\n%s" % "=" * 80)
    print("  P90 PURE CASCADE — MULTI-PAIR BACKTEST SUMMARY")
    print("=" * 80)
    ...
    with open(report_dir / "p90_pure_all_pairs.json", "w") as f:
