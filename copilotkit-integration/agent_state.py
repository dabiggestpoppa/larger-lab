"""
CopilotKit Agent State Manager for Hermes + OpenClaw
Provides shared state and agent control via Python SDK pattern
"""
import json
...
@dataclass
class AgentState:
    """Shared state for Hermes + OpenClaw agents"""
    # Hermes state
...
class CopilotKitAgent:
    """Python SDK-style agent controller for CopilotKit integration"""
...
def get_shared_state() -> dict:
    """Get combined state from both agents"""
    return {
...
if __name__ == "__main__":
    ...
    # Print shared state
    print(json.dumps(get_shared_state(), indent=2))
