...
Read the report at memory-bank/self-heal-report.md.
...
1. Check if memory-bank/ has session-*.md files older than 14 days.
...
2. Count session files in memory-bank/ and shared-conversations/chat-archive/ older than 30 days.
...
IF maintenance needed:
  Reply:
  "ECHO DEEP ACTION:
   - [action 1 — e.g., archive 12 session files from memory-bank/]
