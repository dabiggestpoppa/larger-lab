...
# Find all return statements after the marker
search = 'return "\\n".join(lines)'
pos = start_idx
returns = []
while True:
...
print(f"Found {len(returns)} return statements after marker")
for i, rpos in enumerate(returns[:3]):
    print(f"  #{i+1} at {rpos}: ...{content[rpos-30:rpos+30]!r}...")

# Use the first one
if returns:
    end_idx = returns[0] + len(search)
    print(f"Using return at {returns[0]}, end_idx={end_idx}")
    
    new_handler = r'''        # ── Default: analyze message content and respond dynamically ──
    ...
    print("DONE - default handler replaced")
else:
    print("ERROR: No return statements found")
