...
# Tasks/files to ignore (known-good third-party or already-handled)
IGNORED_TASKS = {
...
def is_ignored(name):
    ...
    # Split by backslash and check each path component
    parts = name.replace("\\", "/").split("/")
    for part in parts:
        part = part.strip()
        if not part:
            continue
        # Check if this part starts with any ignored prefix
        for ign in IGNORED_TASKS:
            if part == ign or part.startswith(ign + " ") or part.startswith(ign + "-"):
                return True
    return False
...
def main():
    ...
    print("\n[1] Scheduled Tasks (non-Microsoft)")
    tasks = get_tasks()
    cur_tasks = set()
    shown = 0
    for t in tasks:
        ...
        shown += 1
        st = t.get("status", "?")
        ra = t.get("run as user", "?")
        cmd = t.get("task to run", "?")[:80]
        ...
        acl = check_acl(n)
        if acl == "RESTRICTED":
            alerts.append("RESTRICTED ACL: " + n)
            print("    DANGER RESTRICTED ACL")
    for old in state.get("known_tasks", {}):
        if old not in cur_tasks:
            print("  INFO REMOVED:", old)
    if shown == 0:
        print("  (none)")

    print("\n[2] Startup Folder")
    su = get_startup()
    cur_su = {e["name"] for e in su}
    for e in su:
        if e["name"] not in state.get("known_startup", []):
            alerts.append("NEW STARTUP: " + e["name"])
            print("  WARN NEW:", e["name"], "(" + str(e["size"]) + " bytes)")
        else:
            print("  OK", e["name"])
    for old in state.get("known_startup", []):
        if old not in cur_su:
            print("  INFO REMOVED:", old)
