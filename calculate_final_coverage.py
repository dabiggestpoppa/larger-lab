...
# Load all normalized files (after re-normalization)
...
print(f"Target window expected hours (no weekends, no holidays): {len(expected_target_no_holiday)}")
print(f"Common window expected hours (no weekends, no holidays): {len(expected_common_no_holiday)}")
...
coverage_v3 = {
    'target_window': {
        ...
        'expected_hours_no_weekends_holidays': len(expected_target_no_holiday)
    },
    'common_window': {
        'start': COMMON_START.isoformat(),
        'end': COMMON_END.isoformat(),
        'expected_hours_no_weekends_holidays': len(expected_common_no_holiday),
