...
# Test C+D override: AR gate removed (999 = no filter), lower triggers
TEST_CD_TIER_CONFIG: Dict[str, Dict[str, float]] = {
    "T1": {"ar_max": 999.0, "au": 6.0, "trigger": 8.0},
    "T2": {"ar_max": 999.0, "au": 8.0, "trigger": 10.0},
    "T3": {"ar_max": 999.0, "au": 10.0, "trigger": 13.0},
}
...
class SymmetryTrapEngine:
    ...
    def process_bar(self, bar: Bar) -> Optional[TradeSignal]:
        ...
        # 80% Kill Switch: REMOVED (dead code — never triggered in testing)
        # DZ: Flat 20-50% for all loops (dynamic 32% floor removed — zero impact)
        # Reference: cerebus_qa_recap.md Q8 (DZ pullback), Q9 (DZ)
        elif self.state == EngineState.WAIT_RETRACE:
            # Kill Switch: REMOVED
            # Dynamic DZ: REMOVED — using flat 20-50% for all loops
