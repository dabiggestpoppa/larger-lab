...
class TestManifestRepo:
    def _repo(
        ...
        return store, blob_repo, acq_repo, repo
    ...
    def _seed_acquisition(
        ...
        acq_repo: AcquisitionRepository,
        sha: str,
        acquisition_id: str = "acq-prov",
        **overrides: Any,
    ) -> None:
        """Durable matching acquisition for one blob (I04R1 §5)."""
        kwargs: dict[str, Any] = {
        ...
        kwargs.update(overrides)
        acq_repo.append_acquisition(AcquisitionRecord(**kwargs))

    # -- version 1 -----------------------------------------------------------

    def test_first_manifest_v1(self, tmp_path: Path) -> None:
        _, _, _, repo = self._repo(tmp_path)
    ...
    def test_blob_ref_without_metadata_fails(self, tmp_path: Path) -> None:
        store, _, _, repo = self._repo(tmp_path)
    ...
    def test_blob_ref_verified_succeeds(self, tmp_path: Path) -> None:
        store, blob_repo, acq_repo, repo = self._repo(tmp_path)
        sha = self._seed_blob(store, blob_repo, b"verified")
        self._seed_acquisition(acq_repo, sha)  # I04R1: provenance required
    ...
    def test_manifest_claim_stronger_than_evidence_rejected(
        ...
        self._seed_acquisition(acq_repo, blob.blob_sha256)  # provenance passes
        with pytest.raises(CatalogIntegrityError):
            repo.append_partition_manifest(
                _manifest(
    ...
    def test_coverage_separate_from_integrity(self, tmp_path: Path) -> None:
        ...
        self._seed_acquisition(acq_repo, sha)
        result = repo.append_partition_manifest(
            _manifest(
                blob_refs=[sha],
                coverage_state="PARTIAL",
