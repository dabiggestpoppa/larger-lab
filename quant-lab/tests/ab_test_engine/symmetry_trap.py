...
class SymmetryTrapEngine:
    ...
    def process_bar(self, bar: Bar) -> Optional[TradeSignal]:
        ...
        # ── PURE OPTION B: 4h timeout DISABLED ──
        # (No timeout — loops continue until entry or session end)
        ...
        # PURE OPTION B: 80% Kill Switch DISABLED
        # PURE OPTION B: Flat 20-50% DZ for ALL loops (no dynamic 32% floor)
        # Reference: cerebus_qa_recap.md Q8 (DZ pullback), Q9 (DZ)
        elif self.state == EngineState.WAIT_RETRACE:
            # Kill Switch check: DISABLED for Pure Option B
            # (kill switch level is still set in SEARCH but never evaluated)

            # ── Flat DZ Thresholds (Pure Option B: same for all loops) ──
            # All loops use 20%-50% retracement zone
        ...
        # PURE OPTION B: Kill Switch DISABLED in WAIT_OCC
