...
class SubConfig(StrategyConfig, frozen=True):
...
class SubStrategy(Strategy):
    ...
    def on_start(self):
        ...
        print('Subscribing to bar_type:', self._bt)
        self.subscribe_bars(self._bt)
        # Also check what data is available
        print('Cached bars:', len(self.cache.bars(self._bt)))
    ...
    def on_stop(self):
        ...
        # Check: did we get the bars?
        cached = self.cache.bars(self._bt)
        print('on_stop: cached bars =', len(cached))

config = BacktestEngineConfig(
    trader_id=TraderId('SUB-001'),
...
engine.add_strategy(strategy=SubStrategy(config=SubConfig()))
