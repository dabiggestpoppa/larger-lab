...
| Current checkpoint | SENSOR-B4-I04R2 COMPLETE — USABLE-ACQUISITION PROVENANCE + STREAMING H3 SEAL (proposed `PASS_SENSOR_B4_I04R2_USABLE_PROVENANCE_SEALED`; then proposed operator acceptance `PASS_SENSOR_B4_I04R1_PROVENANCE_INTEGRITY_SEALED`; then proposed operator acceptance `PASS_SENSOR_B4_I04_ACQUISITION_MANIFEST_REPOSITORY`).  Operator review found ONE remaining provenance contradiction (HOLD_PASS_SENSOR_B4_I04R1_PROVENANCE_INTEGRITY_SEALED_PENDING_I04R2_USABLE_PROVENANCE_SEAL): a failed acquisition (provider_checksum_verified=False + explicit failure evidence) is intentionally retained as FORENSIC FAILURE EVIDENCE, but find_matching_acquisitions() did not distinguish DURABLE ACQUISITION HISTORY from USABLE MANIFEST PROVENANCE, so that failed acquisition could satisfy a LOCAL_HASH_VERIFIED PartitionManifest provenance gate.  I04R2 closed it: ONE authoritative module-level predicate is_usable_manifest_provenance(record) (no blob / failure_ref present / explicitly parsed numeric HTTP failure >= 400 / provider_checksum_verified=False each disqualify; quality flags PARTIAL_INTERVAL/SCHEMA_ADDITIVE/LIMITED never automatically disqualify); separate USABLE read APIs find_usable_matching_acquisitions / has_usable_matching_acquisition route through the one predicate while the FORENSIC reads list_acquisitions_for_blob / find_matching_acquisitions / has_matching_acquisition are unchanged and still return every durable record; PartitionManifestRepository publication consumes the USABLE gate and raises the more specific typed NoUsableAcquisitionProvenance (a MissingAcquisitionProvenance subclass) when matching durable history exists but none is usable; has_earned_h3_proof requires usable eligibility first (verified=True + failure_ref can never prove provider integrity); H3=None may support LOCAL/UNVERIFIED but never PROVIDER; H3=True (earned) supports both; identity matching factored into one shared _identity_matches rule applied BEFORE eligibility (wrong provider/sensor/instrument/granularity regressions remain green); zero-blob manifests unaffected.  Second defect: I04R1 H3 recomputation read the entire decoded blob with an unbounded stream.read(); replaced with bounded incremental streaming — compute_checksum_stream / verify_checksum_stream in storage/checksums.py (SHA256/MD5/CRC32 only; configurable positive chunk_size; never read() without size; non-seekable-safe; starts at current position; caller stream never closed; exact parity with the unchanged byte-based API; CRC32 canonical 8-lowercase-hex leading-zero form; malformed expected value rejected BEFORE any byte is consumed); AcquisitionRepository now recomputes H3 streaming over LocalBlobStore.open_blob — decoded T0A bytes are never materialized solely to verify H3; 8 MiB deterministic logical-stream proof shows reads bounded by chunk_size with exact byte counts (no 1 GiB allocation).  Historical I04/I04R1 evidence untouched (chronology: I04 original -> operator review -> I04R1 repair -> operator review -> I04R2 repair).  I04R2A-D sealed with 48 new tests (usable-provenance 29 + streaming checksum 18 + machine evidence 1; storage 536 -> 584); deterministic machine evidence BLOC_04_I04R2_USABLE_PROVENANCE_MATRIX.json (8 truth cases; byte-stable across regeneration); P1-P5 crash matrix and 8-writer CAS remain green; immutable Parquet behavior unchanged.  Storage 582 nodes (+48); full suite 1961 passed / 0 failed / 3 skipped (1964 collected); ruff clean on changed scope; changed-scope mypy clean; network 0; provider code unchanged; no T0B / DuckDB / PostgreSQL / SourceRevision registry / resume advancement / provider integration.  STORAGE_MODEL_CONTRACTS_READY=TRUE; CONTENT_ADDRESSING_READY=TRUE; ATOMIC_FILESYSTEM_BACKEND_READY=TRUE; T0A_BLOB_BACKEND_IMPLEMENTED=TRUE; BLOB_METADATA_REPOSITORY_IMPLEMENTED=TRUE; ACQUISITION_REPOSITORY_IMPLEMENTED=TRUE; MANIFEST_REPOSITORY_IMPLEMENTED=TRUE; CURRENT_POINTER_SEMANTICS_READY=TRUE; T0A_EVIDENCE_PIPELINE_COMPLETE=TRUE; BLOC3_T0A_INTEGRATION_COMPLETE=FALSE (I14); T0B_STORAGE_IMPLEMENTED=FALSE; SOURCE_REVISION_REGISTRY_IMPLEMENTED=FALSE; DURABLE_RESUME_IMPLEMENTED=FALSE; next_checkpoint_authorized=FALSE; recommended next SENSOR-B4-I05 RAW PROJECTION + LINEAGE — NOT authorized and NOT started
| Previous checkpoint | SENSOR-B4-I04 COMPLETE — ACQUISITION + MANIFEST REPOSITORY (proposed `PASS_SENSOR_B4_I04_ACQUISITION_MANIFEST_REPOSITORY`): durable EvidenceBlob metadata + AcquisitionRecord + append-only PartitionManifest catalog with transactional current pointer, partition lock and expected-current CAS; 58 tests; storage 490 nodes; full suite 1867 passed / 0 failed / 3 skipped; historical I04 evidence preserved (I04R1 does not rewrite it) |
