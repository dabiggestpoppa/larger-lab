...
| Current checkpoint | SENSOR-B4-I05R4 COMPLETE — EVIDENCE IMMUTABILITY + SEALED VERIFIER + SERVICE RETRY SEAL (proposed PASS_SENSOR_B4_I05R4_EVIDENCE_INTERFACE_RETRY_SEALED; then proposed operator acceptance PASS_SENSOR_B4_I05R3_LINEAGE_IDENTITY_TIME_SEALED, PASS_SENSOR_B4_I05R2_FAIL_CLOSED_PUBLIC_API_SEALED, PASS_SENSOR_B4_I05R1_DURABLE_END_TO_END_LINEAGE_SEALED and PASS_SENSOR_B4_I05_RAW_PROJECTION_LINEAGE). Operator HOLD + Four-Dimension Audit findings closed: (A) physical T0B re-verification in the lineage commit path is now UNCONDITIONAL — the hasattr duck-typed optional branch is deleted; a runtime_checkable ProjectionArtifactVerifier Protocol (get + verify_physical) is enforced at ProjectionLineageRepository CONSTRUCTION (typed LineageConfigurationError before any lineage can exist) and verify_physical runs on every first/idempotent commit with failure propagation and no cached success (spy-proven). (B) evidence tests are READ-ONLY: I05R3/I05R2/I05R1 matrix tests generate canonical bytes in memory (stable_evidence_bytes), compare generation 1 == generation 2 == COMMITTED artifact bytes, and never write the evidence tree — a behavior change now fails loudly instead of silently rewriting governance history; post-suite git diff on the evidence tree is clean with the tracked aggregate hash identical to the pre-checkpoint baseline (b3b66e5e...); I05R4 matrices were published once explicitly and are verified read-only. (C) retry is no longer defeated by clock movement: ProjectionContextRepository.commit compares ALL scientific/structural immutable fields and excludes ONLY created_at (first-seen operational metadata, preserved unrewritten); identical scientific re-commit returns the first record; any scientific difference still raises ProjectionIdentityConflict; the REAL T0BProjectionService retried across T1->T2 with repositories restarted from disk completes idempotently; the context-before-lineage crash state is proven NOT resolver-visible (fails closed) and the partial-chain retry completes the missing lineage while preserving T1 context and every durable intermediate byte; alternate lineage_manifest_id retry and changed projection bytes remain typed conflicts; the recovered chain survives a full restart. Audit items 4/5 closed narrowly (portable tmp_path roots replace C:/tmp_r3e_proj and C:/tmp_e2e_proj; dynamic __import__ noise removed); items 6-10 recorded as DESIGN_DEBT_PARKED (PROJECTIONS_MODULE_DECOMPOSITION, TEST_FIXTURE_CONSOLIDATION, GET_BY_PROJECTION_RETENTION, JSON_ENTRY_CLEANUP, CONTEXT_MODEL_PYDANTIC) — no broad refactor performed. Deterministic machine evidence BLOC_04_I05R4_EVIDENCE_IMMUTABILITY_MATRIX.json (5 cases) + BLOC_04_I05R4_VERIFIER_INTERFACE_MATRIX.json (4 cases) + BLOC_04_I05R4_SERVICE_RETRY_MATRIX.json (8 cases), byte-stable and read-only verified; storage 872 passed / 3 skipped (875 collected); full suite 2251 passed / 0 failed / 4 skipped (2255 collected); fresh pre-repair baseline recorded (855 storage / 2234 full); ruff clean; mypy changed scope clean (pre-existing planner.py:79 baseline only); network 0; provider code unchanged; I06 NOT authorized and NOT started. |
