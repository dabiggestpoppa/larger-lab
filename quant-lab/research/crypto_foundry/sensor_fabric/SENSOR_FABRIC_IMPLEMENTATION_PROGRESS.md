...
| Current checkpoint | SENSOR-B4-I05 COMPLETE — RAW PROJECTION + LINEAGE LAYER (proposed PASS_SENSOR_B4_I05_RAW_PROJECTION_LINEAGE). T0B projection schema registry (schema_key = SHA256(id@version), schema_fingerprint = structural SHA256, reserved _t0_ rejection, semver strict); immutable provider-native Parquet writer with staging/verify/publish durability; ProjectionArtifactRepository (immutable catalog); ProjectionLineageRepository (multi-blob lineage, source_order contiguous, usable-provenance gate); PartitionManifestRepository projection_ref validation via injected ProjectionLineageResolver Protocol (fail closed when unconfigured). Storage 660 passed / 2 skipped; full suite 2039 passed / 0 failed / 3 skipped; ruff clean; mypy changed scope clean; network 0; provider code unchanged; I06 NOT started. T0A_EVIDENCE_PIPELINE_COMPLETE=TRUE; T0B_PROJECTION_SCHEMA_READY=TRUE; T0B_PHYSICAL_PROJECTION_WRITER_READY=TRUE; T0B_PROJECTION_CATALOG_IMPLEMENTED=TRUE; T0B_LINEAGE_REPOSITORY_IMPLEMENTED=TRUE; T0B_TO_T0A_LINEAGE_COMPLETE=TRUE; T0B_STORAGE_IMPLEMENTED=TRUE; SOURCE_REVISION_REGISTRY_IMPLEMENTED=FALSE; DURABLE_RESUME_IMPLEMENTED=FALSE; next_checkpoint_authorized=FALSE; recommended next SENSOR-B4-I06 SOURCE REVISION / MUTATION REGISTRY — NOT authorized and NOT started | SENSOR-B4-I04 COMPLETE — ACQUISITION + MANIFEST REPOSITORY (proposed `PASS_SENSOR_B4_I04_ACQUISITION_MANIFEST_REPOSITORY`): durable EvidenceBlob metadata + AcquisitionRecord + append-only PartitionManifest catalog with transactional current pointer, partition lock and expected-current CAS; 58 tests; storage 490 nodes; full suite 1867 passed / 0 failed / 3 skipped; historical I04 evidence preserved (I04R1 does not rewrite it) |
