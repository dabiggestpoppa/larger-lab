...
| Current checkpoint | SENSOR-B4-I04R1 COMPLETE — ACQUISITION-PROVENANCE + H3-INTEGRITY + POINTER-TRUTH SEAL (proposed `PASS_SENSOR_B4_I04R1_PROVENANCE_INTEGRITY_SEALED`, then proposed operator acceptance `PASS_SENSOR_B4_I04_ACQUISITION_MANIFEST_REPOSITORY`).  Operator review found four TRUTH-SEAM classes (HOLD_PASS_SENSOR_B4_I04_ACQUISITION_MANIFEST_REPOSITORY_PENDING_I04R1_PROVENANCE_INTEGRITY_SEAL); I04R1 closed them: A) every non-empty PartitionManifest blob_ref now requires >=1 durable matching AcquisitionRecord (provider/venue/sensor_family/native_instrument + native granularity where both known) — blob durability → blob metadata → acquisition → manifest is now enforced (MissingAcquisitionProvenance; byte identity never transfers provider/sensor identity; zero-blob manifests remain valid); B) provider checksums (H3) are recomputed over the EXACT decoded T0A source bytes before acquisition publication (SHA256/MD5/CRC32 explicit, never inferred; verified=True requires a match, verified=False requires a real mismatch — ProviderChecksumClaimConflict otherwise; a persisted mismatch is retained ONLY as explicit failure evidence; verified=True with no blob rejected), BlobMetadataRepository refuses caller-set PROVIDER_HASH_VERIFIED metadata (capped at LOCAL_HASH_VERIFIED), and a PROVIDER_HASH_VERIFIED manifest claim requires earned H3 on EVERY blob_ref (UnearnedProviderIntegrityClaim); C) blobless outcomes require closed explicit failure evidence (failure_ref or an explicitly parsed numeric HTTP failure code — "OK"/"SUCCESS"/"CURRENT_ONLY" never masquerade as failure) and a narrow non-secret acquisition metadata boundary refuses secret-bearing endpoint_host/endpoint_path/request_family/source_locator values before persistence (reject, never redact; error text names the KEY never the VALUE); D) the current pointer is a CLOSED strictly-typed JSON contract (explicit schema_version=1; nonempty-string partition_key/partition_manifest_id never coerced from numbers; manifest_version integer >= 1, bool/str/float rejected; tz-aware ISO-8601 updated_at; unknown fields → CurrentPointerCorrupt), reads are bound to the requested partition_key (the physical locator hash is never authoritative) and pointer ancestry must equal the manifest's supersedes_manifest_id.  Historical I04 evidence untouched (chronology: I04 original → operator review → I04R1 repair).  I04R1A-D sealed with 46 new tests (provenance gate 9 + H3 11 + blobless/secrets 10 + closed pointer 13 + machine evidence 2; storage 490 → 536); deterministic machine evidence BLOC_04_I04R1_PROVENANCE_MATRIX.json + BLOC_04_I04R1_POINTER_SCHEMA.json (byte-stable across regeneration); P1-P5 crash matrix and 8-writer CAS remain green; immutable Parquet behavior unchanged.  Storage 536 nodes (+46); full suite 1913 passed / 0 failed / 3 skipped (1916 collected); ruff clean on changed scope; changed-scope mypy clean (only pre-existing planner.py:79 baseline + yaml-stub noise); network 0; provider code unchanged; no T0B / DuckDB / PostgreSQL / SourceRevision registry / resume advancement / provider integration.  STORAGE_MODEL_CONTRACTS_READY=TRUE; CONTENT_ADDRESSING_READY=TRUE; ATOMIC_FILESYSTEM_BACKEND_READY=TRUE; T0A_BLOB_BACKEND_IMPLEMENTED=TRUE; BLOB_METADATA_REPOSITORY_IMPLEMENTED=TRUE; ACQUISITION_REPOSITORY_IMPLEMENTED=TRUE; MANIFEST_REPOSITORY_IMPLEMENTED=TRUE; CURRENT_POINTER_SEMANTICS_READY=TRUE; T0A_EVIDENCE_PIPELINE_COMPLETE=TRUE; BLOC3_T0A_INTEGRATION_COMPLETE=FALSE (I14); T0B_STORAGE_IMPLEMENTED=FALSE; SOURCE_REVISION_REGISTRY_IMPLEMENTED=FALSE; DURABLE_RESUME_IMPLEMENTED=FALSE; I05 NOT started 
