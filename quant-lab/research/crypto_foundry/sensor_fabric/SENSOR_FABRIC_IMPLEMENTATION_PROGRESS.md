...
| Current checkpoint | SENSOR-B4-I05R2 COMPLETE — FAIL-CLOSED PROJECTION + LINEAGE PUBLIC API SEAL (proposed PASS_SENSOR_B4_I05R2_FAIL_CLOSED_PUBLIC_API_SEALED; then proposed operator acceptance PASS_SENSOR_B4_I05R1_DURABLE_END_TO_END_LINEAGE_SEALED and PASS_SENSOR_B4_I05_RAW_PROJECTION_LINEAGE). Operator HOLD closed: (A) ProjectionArtifactRepository has NO verification bypass — projection_root + schema_registry mandatory, verify_physical removed, physical verification reads the file's own Parquet schema via ParquetFile (no Hive dataset-discovery columns) and requires EXACT equality with registered provider_native_schema + T0_METADATA_SCHEMA, idempotent commits re-verify physical truth; (B) ProjectionLineageRepository has NO optional proof dependencies — all five (blob store, blob metadata, acquisitions, artifact, context) mandatory at construction (typed LineageConfigurationError), missing artifact → ProjectionArtifactMissing, missing context → ProjectionContextMissing, source-list agreement + provider/venue/sensor/instrument identity UNCONDITIONAL in the commit path; (C) exact physical truth: resolver re-proves at read time full schema equality, T0 constant VALUES per row, contiguous row ordinals, physical row lineage (single-source exact, multi-source declared-pairs-only, one-sided rejected) — writer history is void; (D) PartitionManifest rejects duplicate projection_refs at model level and DurableJsonCatalog's AFTER_WRITE_BEFORE_FSYNC fault point now genuinely precedes ALL file fsyncs (open → write → flush → fault → the ONE fsync → close → verify → publish → dir fsync), proven by an fsync spy. 35 adversarial tests (bypass, native-schema attacks with correct SHAs, T0-value attacks, row-lineage attacks, duplicate refs, crash-boundary truth, restart regression); deterministic machine evidence BLOC_04_I05R2_PUBLIC_API_MATRIX.json (7 cases) + BLOC_04_I05R2_PHYSICAL_SCHEMA_MATRIX.json (11 cases) + BLOC_04_I05R2_CRASH_BOUNDARY_MATRIX.json (7 boundaries with spy-counted fsyncs); storage 828 passed / 3 skipped (831 collected); full suite 2207 passed / 0 failed / 4 skipped (2211 collected); fresh pre-repair baseline recorded (776 storage / 2155 full); ruff clean; mypy changed scope clean (pre-existing planner.py:79 baseline only); network 0; provider code unchanged; I06 NOT authorized and NOT started. | Operator HOLD closed: all immutable projection catalogs (schema/artifact/context/lineage) now persist through the shared DurableJsonCatalog primitive (staged write -> fsync -> reopen/verify -> no-clobber publish -> parent-dir fsync) with hashed physical keys (sha256(utf8(logical_id)) + ".json"), physical-key binding on reload, and corruption fail-closed (ProjectionSchemaCatalogCorrupt / ProjectionArtifactCatalogCorrupt / ProjectionLineageCatalogCorrupt — never a silent skip); schema fingerprint now covers the FULL ordered required T0 metadata schema with lossless Arrow descriptors and fail-closed unsupported types; writer preconditions (reserved _t0_ row keys, quality flags, source hashes, registered-schema gate, exact native field contract, RowProjectionLineage ownership) all validated BEFORE physical publication; ProjectionArtifactRepository physically verifies SHA/row-count/schema/T0 columns before first commit and VALID is earned; ProjectionLineageRepository.commit enforces the full §11 source-truth set (durable metadata + physical blob verification + usable identity-matched acquisitions, manifest-id argument binding, both-or-none inclusive row bounds, idempotence including row bounds); production ProjectionLineageResolver validates manifest projection_refs end to end (artifact+context+lineage+physical+schema+sources+partition match+source visibility) — no stub on the acceptance path; T0BProjectionService earns the complete chain and stops before manifests; restart proof, corruption restart matrix (schema/projection/artifact/context/lineage), catalog crash matrix and concurrent-writer proof sealed. Storage 776 passed / 3 skipped (779 collected); full suite 2155 passed / 0 failed / 4 skipped (2159 collected); fresh pre-repair baseline recorded (663 storage / 2042 full) because 6160960d postdated the I05 evidence freeze; ruff clean; mypy changed scope clean (pre-existing planner.py:79 baseline only); network 0; provider code unchanged; I06 NOT authorized and NOT started. |
...
| Operator review state | RATIFIED — Bloc 2 ratified; Bloc 3 COMPLETE + OPERATOR_ACCEPTED + FROZEN (SENSOR-B3-I11R1-RATIFY: PASS_SENSOR_B3_I11R1_HANDOFF_CONSISTENCY_SEALED = OPERATOR_ACCEPTED, PASS_BLOC_03_IMPLEMENTATION = OPERATOR_ACCEPTED, BLOC_03_IMPLEMENTATION_COMPLETE=TRUE, BLOC_03_FROZEN=TRUE, NETWORK_VALIDATION=PASS, REAL_PROVIDER_ADAPTERS=4, PRODUCTION_PATHS=17/17, PHYSICAL_PRODUCTION_SYMBOL_CHECKS=18/18); BLOC_04_PLAN = PASS_BLOC_04_PLAN_FROZEN; SENSOR-B4-I01R1-RATIFY: PASS_SENSOR_B4_I01R1_STORAGE_CONTRACTS_SEALED = OPERATOR_ACCEPTED, PASS_SENSOR_B4_I01_STORAGE_CONTRACTS_FROZEN = OPERATOR_ACCEPTED; authorized SENSOR-B4-I02 CONTENT ADDRESSING + PATHS + CHECKSUMS ONLY — addresses/identity mechanics, NO persistence yet; SENSOR-B4-I02 COMPLETE — proposed PASS_SENSOR_B4_I02_CONTENT_ADDRESSING_PATHS_CHECKSUMS; SENSOR-B4-I02R1 COMPLETE — proposed PASS_SENSOR_B4_I02R1_CANONICAL_PATHS_SEALED; SENSOR-B4-I02R1-RATIFY (this commit): OPERATOR_ACCEPTED both verdicts, pytest-node truth reconciled (storage 259 = enums 18 + models 81 + serialization 16 + checksums 35 + paths 109; full collect 1639 = 1638 run + 1 live-smoke skip; I02R1A +20 nodes, I02R1B net +2, total +22), historical "35/272" wording recorded as overstated, original I02/I02R1 evidence preserved; authorized SENSOR-B4-I03 ATOMIC FILESYSTEM BACKEND ONLY — I04 NOT authorized; SENSOR-B4-I03 COMPLETE — proposed PASS_SENSOR_B4_I03_ATOMIC_FILESYSTEM_BACKEND; SENSOR-B4-I03R1 COMPLETE — proposed PASS_SENSOR_B4_I03R1_DURABILITY_NAMESPACE_SEALED; SENSOR-B4-I03R1-RATIFY (this commit): OPERATOR_ACCEPTED PASS_SENSOR_B4_I03R1_DURABILITY_NAMESPACE_SEALED + PASS_SENSOR_B4_I03_ATOMIC_FILESYSTEM_BACKEND (ATOMIC_FILESYSTEM_BACKEND_READY=TRUE; T0A_BLOB_BACKEND_IMPLEMENTED=TRUE); independent re-verification at head 1434ad29 (full suite 1803 passed / 0 failed / 3 skipped; storage 426 nodes; zero network); I04 NOT authorized; SENSOR-B4-I04-AUTH (this commit): operator confirms I03R1 + I03 acceptance, records ACTUAL ROOT POLICY prose reconciliation (missing root may be created durably from the deepest existing ancestor; existing non-directory fails; no blind recursive mkdir), and authorizes SENSOR-B4-I04 ACQUISITION + MANIFEST REPOSITORY ONLY — I05 NOT authorized; SENSOR-B4-I04 COMPLETE (this commit): proposed PASS_SENSOR_B4_I04_ACQUISITION_MANIFEST_REPOSITORY; storage 490 nodes (+64); full suite 1867 passed / 0 failed / 3 skipped (1870 collected); deterministic machine evidence BLOC_04_I04_CATALOG_SCHEMAS.json + BLOC_04_I04_MANIFEST_CONCURRENCY.json sealed; T0A_EVIDENCE_PIPELINE_COMPLETE=TRUE (storage-layer pipeline); BLOC3_T0A_INTEGRATION_COMPLETE=FALSE (I14); I05 NOT authorized and NOT started; SENSOR-B4-I04R1 (this commit): operator hold HOLD_PASS_SENSOR_B4_I04_ACQUISITION_MANIFEST_REPOSITORY_PENDING_I04R1_PROVENANCE_INTEGRITY_SEAL closed by the microseal — acquisition provenance gate, H3 recomputation, blobless/secret closure, closed pointer schema; proposed PASS_SENSOR_B4_I04R1_PROVENANCE_INTEGRITY_SEALED then proposed operator acceptance PASS_SENSOR_B4_I04_ACQUISITION_MANIFEST_REPOSITORY; storage 536 nodes (+46); full suite 1913 passed / 0 failed / 3 skipped (1916 collected); deterministic machine evidence BLOC_04_I04R1_PROVENANCE_MATRIX.json + BLOC_04_I04R1_POINTER_SCHEMA.json sealed; T0A_EVIDENCE_PIPELINE_COMPLETE=TRUE; I05 NOT authorized and NOT started; SENSOR-B4-I04R2-RATIFY (this commit): OPERATOR_ACCEPTED PASS_SENSOR_B4_I04R2_USABLE_PROVENANCE_SEALED + PASS_SENSOR_B4_I04R1_PROVENANCE_INTEGRITY_SEALED + PASS_SENSOR_B4_I04_ACQUISITION_MANIFEST_REPOSITORY; T0A_EVIDENCE_PIPELINE_COMPLETE=TRUE; test-count wording reconciled (582 passed storage tests / 584 collected; 1961 passed / 1964 collected full suite); authorized SENSOR-B4-I05 RAW PROJECTION + LINEAGE ONLY — I06 NOT authorized; SENSOR-B4-I05 COMPLETE: proposed PASS_SENSOR_B4_I05_RAW_PROJECTION_LINEAGE; operator HOLD HOLD_PASS_SENSOR_B4_I05_RAW_PROJECTION_LINEAGE_PENDING_I05R1_DURABLE_END_TO_END_LINEAGE_SEAL closed by SENSOR-B4-I05R1 (this commit): durable atomic catalogs, safe hashed catalog identities, corruption fail-closed, exact schema fidelity, writer prepublication validation, physically-verifying artifact commit, enforcing lineage commit, production ProjectionLineageResolver + end-to-end manifest integration, restart/corruption/concurrency adversarial proof; proposed PASS_SENSOR_B4_I05R1_DURABLE_END_TO_END_LINEAGE_SEALED; I06 NOT authorized and NOT started; operator HOLD HOLD_PASS_SENSOR_B4_I05R1_DURABLE_END_TO_END_LINEAGE_SEALED_PENDING_I05R2_FAIL_CLOSED_PUBLIC_API_SEAL closed by SENSOR-B4-I05R2 (this commit): fail-closed public API seal — no artifact verification bypass, mandatory lineage proof dependencies, exact physical native-schema + T0-value + row-lineage read-time proof, duplicate projection_refs rejected, AFTER_WRITE_BEFORE_FSYNC crash boundary made truthful (fsync-spy proof); proposed PASS_SENSOR_B4_I05R2_FAIL_CLOSED_PUBLIC_API_SEALED; I06 NOT authorized and NOT started |
...
| next_checkpoint_authorized | FALSE — SENSOR-B4-I04R1 PROVENANCE + INTEGRITY + POINTER-TRUTH SEAL is COMPLETE (proposed PASS_SENSOR_B4_I04R1_PROVENANCE_INTEGRITY_SEALED; then proposed operator acceptance PASS_SENSOR_B4_I04_ACQUISITION_MANIFEST_REPOSITORY — awaiting operator review).  Recommended next: SENSOR-B4-I05 RAW PROJECTION + LINEAGE LAYER — NOT authorized and NOT started. |
