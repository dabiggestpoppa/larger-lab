...
| Current checkpoint | SENSOR-B4-I05R3 COMPLETE — LINEAGE-MANIFEST IDENTITY + IDEMPOTENT REVALIDATION + UTC TRUTH SEAL (proposed PASS_SENSOR_B4_I05R3_LINEAGE_IDENTITY_TIME_SEALED; then proposed operator acceptance PASS_SENSOR_B4_I05R2_FAIL_CLOSED_PUBLIC_API_SEALED, PASS_SENSOR_B4_I05R1_DURABLE_END_TO_END_LINEAGE_SEALED and PASS_SENSOR_B4_I05_RAW_PROJECTION_LINEAGE). Operator HOLD closed: (A) ProjectionCatalogRecord.lineage_manifest_id now BINDS lineage publication — the three-way rule (commit argument == every entry's lineage_manifest_id == context.lineage_manifest_id) is proven BEFORE durable publication; an alternate lineage manifest for an already-bound projection fails typed (LineageContextBindingConflict) and never reaches disk; one ProjectionCatalogRecord has exactly one authoritative lineage manifest (no lineage revision semantics in I05 v1); repository reload fails closed (LineageProjectionIdentityConflict) if multiple committed manifests claim one projection; get_by_projection refuses to concatenate competing manifests; the production resolver resolves lineage through context.lineage_manifest_id directly. (B) Idempotent lineage re-commit no longer returns cached truth before re-proof — every commit (idempotent or first) re-runs artifact existence, artifact source-list agreement, context existence, the three-way binding, physical T0B verification via the new public ProjectionArtifactRepository.verify_physical, and per-entry T0A source verification (durable metadata, physical blob verification, usable identity-matched acquisitions) before success; identical re-commits after source-blob corruption, context loss (restart semantics) and physical T0B corruption ALL FAIL while historical lineage stays durable. (C) ProjectionCatalogRecord rejects naive datetimes for logical_date_start/end, min/max_provider_time and created_at (ProjectionPreconditionError — no silent replace(tzinfo=UTC)); offset-aware non-UTC inputs serialize normalized to UTC (2026-01-01T12:00:00-05:00 -> 2026-01-01T17:00:00+00:00); from_dict rejects persisted naive fragments as catalog corruption; max_provider_time >= min_provider_time enforced when both exist, missing bounds never manufactured; _canonical_utc is a fail-closed backstop. Crash-wording clarification recorded (§20): AFTER_WRITE_BEFORE_FSYNC fires after handle close but before the fsync via fsync_file — ZERO fsyncs before the fault remains the proven invariant; historical I05R2 prose untouched. Deterministic machine evidence BLOC_04_I05R3_LINEAGE_IDENTITY_MATRIX.json (7 cases) + BLOC_04_I05R3_TIME_CONTRACT_MATRIX.json (8 cases), byte-stable across two generations; storage 855 passed / 3 skipped (858 collected); full suite 2234 passed / 0 failed / 4 skipped (2238 collected); ruff clean; mypy changed scope clean (pre-existing planner.py:79 baseline only); network 0; provider code unchanged; I06 NOT authorized and NOT started. |
