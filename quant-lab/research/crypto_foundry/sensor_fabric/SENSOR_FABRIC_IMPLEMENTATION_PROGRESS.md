...
| Current checkpoint | SENSOR-B4-I04 COMPLETE — ACQUISITION + MANIFEST REPOSITORY (proposed `PASS_SENSOR_B4_I04_ACQUISITION_MANIFEST_REPOSITORY`).  I04A reconciled the AcquisitionRecord persistence schema with the frozen Bloc-3 handoff: adapter_capability_version, endpoint_host/endpoint_path/request_family, actual_start/actual_end kept SEPARATE from requested_start/requested_end (never inferred), schema_state, evidence_ref, provider_checksum_algorithm/value/verified triple (algorithm never inferred from digest length), request_started_at/response_observed_at/ingested_at distinct (ingested_at never used as market availability time), resume/page facts preserved without active advancement; secrets (auth signature, API key, Authorization, Cookie, bearer, private account id) never persisted.  I04B implemented the durable append-only EvidenceBlob metadata + AcquisitionRecord catalog as immutable Parquet fragments under <t0_root>/catalogs/manifests/{blobs,acquisitions}/ with stable explicit Arrow schemas (no pandas inference), I03R1 durability doctrine (stage → flush → fsync → reopen/verify schema+records → no-clobber publish → parent fsync), blob-metadata-before-acquisition ordering, physical-blob-verified-before-metadata-commit (successful acquisition never points at missing/unverified bytes), idempotent reuse vs typed immutable conflicts (byte_length/storage_encoding/storage_uri/integrity-defining conflicts never silently overwritten), empty-body-is-blob (b"" valid zero-byte evidence), same-blob-across-distinct-acquisition-ids allowed, acquisition_id immutable (conflicting content → ManifestIdentityConflict-style typed failure).  I04C implemented append-only PartitionManifest complete-snapshot versions under catalogs/manifests/partitions/ (v1 requires no current; vN = current+1 superseding current; no gaps; projection_refs empty/fail-closed pre-I05; every blob_ref referentially verified against physical + metadata evidence; integrity claim never exceeds referenced evidence; coverage and integrity independent) with a partition-scoped atomic-mkdir lock (catalogs/locks/partitions/, ManifestLockHeld fail-closed, no stale-lock auto-delete) + expected-current CAS (stale writers fail ManifestCASConflict, never auto-rebase) and a mutable current pointer (catalogs/current/partitions/, stage+fsync+atomic-replace+parent-fsync, old-or-new visibility, P1-P5 crash matrix: P1 no new manifest / P2 orphan possible / P3 staged pointer / P4 retry re-establishes parent durability / P5 idempotent CAS resolution; orphan fragments never auto-deleted).  I04D sealed with 58 tests (catalog 25 + manifests 27 + concurrency 4 + machine evidence 2): referential integrity, 8-writer same-base race (exactly one current v2, provable winner-in-candidate-set), pointer crash matrix; deterministic machine evidence BLOC_04_I04_CATALOG_SCHEMAS.json + BLOC_04_I04_MANIFEST_CONCURRENCY.json (byte-stable across regeneration).  Storage 490 nodes (+64); full suite 1867 passed / 0 failed / 3 skipped (1870 collected); ruff clean on changed scope; changed-scope mypy clean (only pre-existing planner.py:79 baseline + zstandard stub noise); network 0; provider code unchanged; no T0B / DuckDB / PostgreSQL / SourceRevision registry / resume advancement / provider integration.  STORAGE_MODEL_CONTRACTS_READY=TRUE; CONTENT_ADDRESSING_READY=TRUE; ATOMIC_FILESYSTEM_BACKEND_READY=TRUE; T0A_BLOB_BACKEND_IMPLEMENTED=TRUE; BLOB_METADATA_REPOSITORY_IMPLEMENTED=TRUE; ACQUISITION_REPOSITORY_IMPLEMENTED=TRUE; MANIFEST_REPOSITORY_IMPLEMENTED=TRUE; CURRENT_POINTER_SEMANTICS_READY=TRUE; T0A_EVIDENCE_PIPELINE_COMPLETE=TRUE (storage-layer pipeline); BLOC3_T0A_INTEGRATION_COMPLETE=FALSE (I14); T0B_STORAGE_IMPLEMENTED=FALSE; SOURCE_REVISION_REGISTRY_IMPLEMENTED=FALSE; DURABLE_RESUME_IMPLEMENTED=FALSE; I05 NOT started |
| Previous checkpoint | SENSOR-B4-I04-AUTH COMPLETE (governance) — operator confirms I03/I03R1 acceptance (PASS_SENSOR_B4_I03R1_DURABILITY_NAMESPACE_SEALED = OPERATOR_ACCEPTED; PASS_SENSOR_B4_I03_ATOMIC_FILESYSTEM_BACKEND = OPERATOR_ACCEPTED; ATOMIC_FILESYSTEM_BACKEND_READY=TRUE; T0A_BLOB_BACKEND_IMPLEMENTED=TRUE); I03R1 root-policy prose reconciled chronologically (ACTUAL ROOT POLICY: existing root directory → use it; existing non-directory → fail; missing root → may be created through durable directory-chain creation from the deepest existing ancestor; no blind recursive mkdir + durability assumption); authorizes SENSOR-B4-I04 ACQUISITION + MANIFEST REPOSITORY ONLY; I05 NOT authorized |
...
| next_checkpoint_authorized | FALSE — SENSOR-B4-I04 ACQUISITION + MANIFEST REPOSITORY is COMPLETE (proposed PASS_SENSOR_B4_I04_ACQUISITION_MANIFEST_REPOSITORY, awaiting operator review).  Recommended next: SENSOR-B4-I05 RAW PROJECTION + LINEAGE LAYER — NOT authorized and NOT started. |
