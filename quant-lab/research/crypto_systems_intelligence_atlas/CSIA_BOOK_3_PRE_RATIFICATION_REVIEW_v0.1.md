...
| Can forks remain historical without identity collapse? | **YES** | Fork edges, shared history, and supersession are explicit architecture-history claims. |
...
| `SECURED_BY`, `SETTLES_TO`, `USES_DA`, `SEQUENCED_BY`, `MESSAGES_TO`, and modular decomposition | **RELATIONSHIP_EXTENSION_CANDIDATE** | Validate against Book 1 relationship semantics before any future amendment. |
...
|---|---|---|---|---|---|---|
| D3-1 | What makes a fork the same network continuation? | A) shared genesis only; B) shared history plus state/config continuity; C) operator-specific rule | A is too weak; B preserves history; C permits family nuance but needs governance | Prefer B with typed fork evidence and preserve all alternatives | Yes, as policy amendment | **Yes** for identity ratification |
...
BOOK_1_CONTRACT_AMENDMENT_COUNT = 0 recommended
