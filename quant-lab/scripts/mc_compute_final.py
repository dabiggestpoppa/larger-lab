"""
Monte Carlo: 10,000 sims x 250 trading days (1 year).
Daily return drawn from normal distribution with analytically-derived params.
"""
...
COSTS = 2.9
MC_DAYS = 250  # 1 year of trading
TRADING_DAYS_FULL = 1350  # full backtest period
WEEKS = TRADING_DAYS_FULL / 5
...
for name, data in v4b.items():
    ...
    trades_per_day = total_trades / TRADING_DAYS_FULL
    ...
    # Daily PnL: compound of Poisson(N) trades
    # E[day] = E[N] * ev_trade
    # Var[day] = E[N] * (var_trade + ev_trade^2)  [since Var[N]=E[N] for Poisson]
    ev_daily = trades_per_day * ev_trade
    var_daily = trades_per_day * (var_trade + ev_trade**2)
    std_daily = math.sqrt(max(var_daily, 0.01))

    # Monte Carlo: 10,000 sims x 250 days
    random.seed(42)
    n_sims = 10000
    mean_dailies = []
    median_dailies = []
    ...
    for _ in range(n_sims):
        ...
        mean_dailies.append(statistics.mean(daily_pnls))
        median_dailies.append(statistics.median(daily_pnls))
    ...
    overall_mean_daily = statistics.mean(mean_dailies)
    overall_median_daily = statistics.mean(median_dailies)
    ...
    strategies[name] = {
        ...
        'mean_daily_return': round(overall_mean_daily, 2),
        'median_daily_return': round(overall_median_daily, 2),
