...
for name, data in v4b.items():
    ...
    # Analytical approach: compute MC metrics from trade distribution
    # Mean daily return = trades_per_day * [(WR * avg_win) - ((1-WR) * avg_loss) - COSTS]
    mean_daily = trades_per_day * ((wr * avg_win) - ((1 - wr) * avg_loss) - COSTS)

    # Std dev of single trade
    ev_trade = (wr * avg_win) - ((1 - wr) * avg_loss) - COSTS
    var_trade = wr * (avg_win - ev_trade)**2 + (1-wr) * (-avg_loss - ev_trade)**2
    std_trade = math.sqrt(var_trade) if var_trade > 0 else 0

    # Std dev of daily return (central limit theorem)
    std_daily = std_trade * math.sqrt(trades_per_day) if trades_per_day > 0 else 0

    # For MC, use analytical + simulated approach with fewer sims
    # Simulate 10,000 daily returns (not full trade sequences)
    random.seed(42)
    n_sims = 10000

    # Simulate equity curves using daily return distribution
    daily_returns = []
    max_dds = []
    ...
    for _ in range(n_sims):
        ...
        for day in range(TRADING_DAYS):
            ...
            n_trades = max(0, int(random.gauss(trades_per_day, math.sqrt(max(trades_per_day, 0.01)))))
            day_pnl = 0
