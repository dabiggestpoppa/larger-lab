#!/usr/bin/env python3
"""
DMR (Deep Mean Reversion) MT5 FORWARD TEST — LIVE
Account: 1114712 | Server: OxSecurities-Demo
...
LOG_FILE = RESULTS_DIR / "dmr_forward_test_log.csv"
STATE_FILE = RESULTS_DIR / "dmr_forward_test_state.json"
...
def place_order(direction, sl, tp):
    ...
    req = {
        ...
        "deviation": 10, "magic": MAGIC_NUMBER, "comment": "DMR_FWD_" + direction,
...
def load_state():
    ...
    return {'today': None, 'trade_placed': False, 'total_trades': 0, 'wins': 0, 'losses': 0, 'pnl': 0.0}
...
def main():
    print("="*60)
    print("DMR FORWARD TEST — LIVE MT5 DEMO")
    print("Account:", LOGIN, "|", SERVER, "|", SYMBOL, "| Lot:", LOT_SIZE)
    print("="*60)
    if not connect(): sys.exit(1)
    state = load_state()
    print("State:", state)
    print("Scanning every 60s | Ctrl+C to stop")
    print("")
    try:
        while True:
            try:
                ...
                direction, p90 = find_p90(today_bars)
                if not direction:
                    print(now.strftime('%H:%M'), "EST — scanning, no P90 signal yet")
                    time.sleep(60); continue
