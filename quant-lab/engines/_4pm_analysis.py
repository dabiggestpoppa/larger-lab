"""
Check how many CSV engine trades happen after 4PM EST.
This is the key remaining difference vs Nautilus.
"""
import sys
...
for dk in sorted(days.keys()):
    ...
    for bar in day_bars:
        ...
        # CSV runner: only break at 4PM if in SEARCH state
        if bar_est_h >= 16 and engine.state == EngineState.SEARCH:
            # Record state at 4PM for analysis
            state_key = engine.state.value
            state_at_4pm[state_key] = state_at_4pm.get(state_key, 0) + 1
            break
...
print(f"=== CSV ENGINE ===")
print(f"Trades: {len(all_trades)} | W: {wins} L: {losses} | WR: {wr:.1f}% | PnL: {total_pnl:+.1f}p")
print(f"\n=== 4PM ANALYSIS ===")
print(f"Entries after 4PM: {entries_after_4pm}")
print(f"Exits after 4PM: {trades_after_4pm}")
print(f"State at 4PM cutoff: {state_at_4pm}")
print(f"\nTotal trades after 4PM: {entries_after_4pm + trades_after_4pm}")
print(f"This explains ~{entries_after_4pm + trades_after_4pm} of the {5084 - 3841} gap vs Nautilus")
