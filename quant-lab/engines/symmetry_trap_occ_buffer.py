"""
CEREBUS FX v4.0 — Symmetry Trap OCC Buffer Engine (Signal-Only)
=================================================================

Derived from Symmetry Trap Engine (symmetry_trap.py).
MAD Directive 2026-06-10: ST+OCC buffer engine — signals only, no execution.

KEY DIFFERENCE from base Symmetry Trap:
  - Base ST: SL = Zero-Buffer Impulse Extreme (profit lock — SL on wrong side of entry)
  - OCC Buffer: SL = Impulse Extreme + Spread Buffer (regular SL — actual stop loss)
  - For LONG:  SL = impulse_high - buffer (below entry = real stop)
  - For SHORT: SL = impulse_low + buffer (above entry = real stop)
  - Buffer = spread_buffer + min_sl_buffer (from asset config)
  - This is a REAL stop loss, not a profit lock

SIGNAL OUTPUT:
  - Writes to occ_buffer_signals.jsonl (separate from ST signals)
  - Same signal format as SymmetryTrap for signal_bot compatibility
  - No order placement — signal only

STATE MACHINE (4 states):
  SEARCH → WAIT_RETRACE → WAIT_OCC → IN_TRADE → (reset to SEARCH)

Author: CEREBUS — MAD Directive 2026-06-10
Mode: Signal-Only / No Execution
"""
...
class SymmetryTrapOCCBufferEngine:
    """
    ...
    def process_bar(self, bar: Bar) -> Optional[TradeSignal]:
        ...
        # Reference: cerebus_qa_recap.md Q8, manual_ontology.md Computable Q3
            ...
            if occ_confirmed:
                ...
                sl_pips = abs(self.entry_price - self.sl_price) / self.pip_size
                self.logger.info(
                    f"ENTRY {'LONG' if self.impulse_direction == TradeDirection.LONG else 'SHORT'} "
                    f"(loop {self.loop_count}): "
                    f"entry={self.entry_price:.5f}, sl={self.sl_price:.5f} ({sl_pips:.1f}p buffer), "
            ...
            if self.impulse_direction == TradeDirection.LONG:
                ...
                if bar.close <= self.sl_price:
                    ...
                    sig = TradeSignal(
                        ...
                        au_used=self.au_pips,
                        timestamp=bar.timestamp,
                        reason=f"OCC Buffer SL hit (close-only) – loop {_loop} complete",
                        loop_count=_loop,
                    )
                    self.signal_log.append(sig)
    ...
    def _classify_tier_by_impulse(self) -> None:
        ...
        Updates self.tier_name, self.au_pips, self.active_au.
        NOTE: self.trigger_pips is NOT updated — trigger stays at T1 value
        for all loops. Only AU changes with tier (for DZ calculation).
        Per June 4 calibration: trigger is fixed at T1 for the session.
        """
