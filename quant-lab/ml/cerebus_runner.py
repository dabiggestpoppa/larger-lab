"""
CEREBUS 24/7 Runner — Single process manages all scanners.
Starts all scanners as subprocesses, monitors them, auto-restarts on crash.
"""
import os
import sys
import time
...
LOG_FILE = REPO_ROOT / "logs" / "cerebus_runner.log"
PID_FILE = REPO_ROOT / ".cerebus_runner.pids"
...
def main():
    log("=" * 60)
    log("CEREBUS 24/7 RUNNER — Starting")
        ...
        while True:
            ...
            for s in SCANNERS:
                ...
                if restart_counts[name] > MAX_RESTARTS:
                    log(f"  ❌ {name} exceeded max restarts — waiting")
