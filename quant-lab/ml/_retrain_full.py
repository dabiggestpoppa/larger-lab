...
Uses calibrated tier/AU configs. No shortcuts. All 30+ features.
"""
...
# Columns to exclude from features (raw OHLCV, intermediates, targets, labels)
EXCLUDE_COLS = {
...
for labels_file in sorted(LABELS_DIR.glob('*_labeled.parquet')):
    ...
    # Select feature columns — exclude string/categorical columns
    label_cols = {'label_25_delivery', 'label_50_delivery', 'rekey_triggered', 'regime_at_time'}
    feat_cols = []
    for c in df.columns:
        if c in EXCLUDE_COLS or c in label_cols:
            continue
        # Skip any non-numeric columns
        if df[c].dtype == object or df[c].dtype.name == 'category' or df[c].dtype.name == 'string':
            continue
        feat_cols.append(c)
