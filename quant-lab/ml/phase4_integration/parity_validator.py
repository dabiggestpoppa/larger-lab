...
def check_parity(asset: str, nt8_results: dict) -> dict:
    """Compare NT8 backtest results against manual benchmarks."""
    ...
    wr_delta = nt8_results.get("win_rate", 0) - bench["wr"]
    if abs(wr_delta) > TOLERANCES["wr"]:
        issues.append(f"WR {nt8_results['win_rate']:.1f}% vs target {bench['wr']}%")

    r_delta = nt8_results.get("avg_r", 0) - bench["r"]
    if abs(r_delta) > TOLERANCES["r"]:
        issues.append(f"Avg R {nt8_results['avg_r']:.2f} vs target {bench['r']}")
...
class ParityValidator:
    ...
    def __init__(self, backtest_metrics: dict, tolerance: float = 0.05):
