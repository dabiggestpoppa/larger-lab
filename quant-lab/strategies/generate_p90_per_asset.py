"""
Generates the CEREBUS P90 V5 PER-ASSET Pine Script.
Run this to produce the .pine file that gets pushed to TradingView.
"""

from p90_v5_per_asset import ASSET_P90_CONFIGS

# ── Build the asset detection block ──────────────────────────────────
detection_lines = []
for key in ASSET_P90_CONFIGS:
    var_name = f"_{key.lower()}"
    detection_lines.append(f"    {var_name}  = str.contains(_sym, \"{key}\")")

# Remove duplicates for overlapping matches (e.g., US500 matches "500")
# We need special handling for indices that could overlap
detection_block = "\n".join(detection_lines)

# ── Build the pip_size resolution ────────────────────────────────────
pip_cases = []
for key, cfg in ASSET_P90_CONFIGS.items():
    var_name = f"_{key.lower()}"
    pip_val = cfg["pip"]
    pip_cases.append(f"    {var_name} ? {pip_val}")

pip_size_block = "\n".join(pip_cases)

# ── Build the P90 threshold resolution (per hour) ───────────────────
# The original uses hour-based thresholds. We scale the base P90 by hour
# using the same ratios as the original EUR/USD config:
#   2-4 AM: 4.1  → ratio = 4.1/4.6 = 0.8913
#   4-6 AM: 4.6  → ratio = 1.0
#   6-8 AM: 4.6  → ratio = 1.0
#   8-10 AM: 5.9 → ratio = 5.9/4.6 = 1.2826
#   10-11 AM: 6.2 → ratio = 6.2/4.6 = 1.3478
hour_ratios = {
    (2, 4): 0.8913,   # 4.1 / 4.6
    (4, 6): 1.0,      # 4.6 / 4.6
    (6, 8): 1.0,      # 4.6 / 4.6
    (8, 10): 1.2826,  # 5.9 / 4.6
    (10, 11): 1.3478, # 6.2 / 4.6
}

# Build P90 threshold function that auto-selects per-asset
p90_threshold_cases = []
for key, cfg in ASSET_P90_CONFIGS.items():
    var_name = f"_{key.lower()}"
    base_p90 = cfg["p90"]
    # Calculate per-hour thresholds
    lines = []
    for (start, end), ratio in hour_ratios.items():
        val = round(base_p90 * ratio, 2)
        lines.append(f"        {var_name} and h >= {start} and h < {end} ? {val}")
    p90_threshold_cases.extend(lines)

p90_threshold_block = "\n".join(p90_threshold_cases)

# ── Build the Pine Script ────────────────────────────────────────────
pine_script = f'''//@version=6
// CEREBUS P90 V5 PER-ASSET — Auto-configured for {len(ASSET_P90_CONFIGS)} assets
// Generated from asset_configs.py — DO NOT EDIT MANUALLY
// Original: CEREBUS 👁️ V5 LIVE PERFECT FORM FIXED v5
// This copy adds per-asset P90 thresholds + auto pip_size detection

strategy("CEREBUS 👁️ P90 V5 PER-ASSET [AUTO]", 
         overlay=true, 
...
// Default P90 thresholds (EUR/USD base — used as fallback)
...
pip_size = {pip_size_block}
    : 0.0001  // fallback default

// ── Per-Asset P90 Threshold Function ────────────────────────────────
// Auto-selects threshold based on detected asset + current EST hour
// Scales EUR/USD hour ratios to each asset's base P90

get_p90_threshold_bull() =>
    h = get_est_hour()
    if h < 2 or h >= 11
        0.0
    else
{p90_threshold_block}
        // Fallback to EUR/USD defaults scaled by asset
        h >= 2 and h < 4 ? p90_bull_2_4am :
        h >= 4 and h < 6 ? p90_bull_4_6am :
        h >= 6 and h < 8 ? p90_bull_6_8am :
        h >= 8 and h < 10 ? p90_bull_8_10am :
        h >= 10 and h < 11 ? p90_bull_10_11am : 0.0

get_p90_threshold_bear() =>
    h = get_est_hour()
    if h < 2 or h >= 11
        0.0
    else
{p90_threshold_block}
        // Fallback to EUR/USD defaults scaled by asset
        h >= 2 and h < 4 ? p90_bear_2_4am :
        h >= 4 and h < 6 ? p90_bear_4_6am :
        h >= 6 and h < 8 ? p90_bear_6_8am :
        h >= 8 and h < 10 ? p90_bear_8_10am :
        h >= 10 and h < 11 ? p90_bear_10_11am : 0.0

// ── Get Active Asset Name for Display ────────────────────────────────
...
p90_bull_threshold = get_p90_threshold_bull()
...
p90_bear_threshold = get_p90_threshold_bear()
...
// ============================================================================
// 📊 VISUALIZATION - ASIAN RANGE + P90 NO-FILTER (SINGLE COLOR)
// ============================================================================
...
// ============================================================================
// 📊 INFO PANEL - TOP RIGHT (P90 ONLY)
// ============================================================================

var table info_table = table.new(position.top_right, 2, 22, bgcolor=color.black, border_width=1)

if barstate.islast
    table.cell(info_table, 0, 0, "CEREBUS P90 PER-ASSET [EST]", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    ...
    table.cell(info_table, 0, 3, "Current Time (EST)", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 3, str.tostring(get_est_hour(), "00") + ":" + str.tostring(get_est_minute(), "00"), text_color=color.white, bgcolor=color.blue, text_size=size.small)
    
    table.cell(info_table, 0, 4, "P90 Signals Today", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 4, str.tostring(signals_today), text_color=color.white, bgcolor=color.green, text_size=size.small)
    
    table.cell(info_table, 0, 5, "In Hold Period", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 5, in_hold_period ? "YES" : "NO", text_color=color.white, bgcolor=in_hold_period ? color.purple : color.green, text_size=size.small)
    
    if not na(last_entry_time)
        mins_since = (time - last_entry_time) / 60000
        table.cell(info_table, 0, 6, "Mins Since Entry", text_color=color.white, bgcolor=color.gray, text_size=size.small)
        table.cell(info_table, 1, 6, str.tostring(mins_since, "#") + " / " + str.tostring(hold_time_minutes), text_color=color.white, bgcolor=mins_since >= hold_time_minutes ? color.green : color.orange, text_size=size.small)
    else
        table.cell(info_table, 0, 6, "Mins Since Entry", text_color=color.white, bgcolor=color.gray, text_size=size.small)
        table.cell(info_table, 1, 6, "N/A", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    
    table.cell(info_table, 0, 7, "Asian Range", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 7, str.tostring(asian_range_pips, "#.##") + " pips", text_color=color.white, bgcolor=ar_status == "GO" ? color.green : ar_status == "CAUTION" ? color.orange : color.red, text_size=size.small)
    
    table.cell(info_table, 0, 8, "AR Status", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 8, ar_status, text_color=color.white, bgcolor=ar_status == "GO" ? color.green : ar_status == "CAUTION" ? color.orange : color.red, text_size=size.small)
    
    table.cell(info_table, 0, 9, "P90 Direction", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 9, p90_direction, text_color=color.white, bgcolor=p90_direction == "LONG" ? color.green : p90_direction == "SHORT" ? color.red : color.gray, text_size=size.small)
    
    table.cell(info_table, 0, 10, "-25% Hit", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 10, entry_ext_25_hit ? "YES" : "NO", text_color=color.white, bgcolor=entry_ext_25_hit ? color.green : color.gray, text_size=size.small)
    
    table.cell(info_table, 0, 11, "-50% Hit", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 11, entry_ext_50_hit ? "YES" : "NO", text_color=color.white, bgcolor=entry_ext_50_hit ? color.green : color.gray, text_size=size.small)
    
    table.cell(info_table, 0, 12, "132% Violation", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 12, entry_violation_triggered ? "YES" : "NO", text_color=color.white, bgcolor=entry_violation_triggered ? color.red : color.green, text_size=size.small)
    
    table.cell(info_table, 0, 13, "Filter Status", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 13, filter_25_50_hit ? "BLOCKED" : "OK", text_color=color.white, bgcolor=filter_25_50_hit ? color.red : color.green, text_size=size.small)
    
    table.cell(info_table, 0, 14, "─── P90P Tracker ───", text_color=color.yellow, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 14, "", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    
    table.cell(info_table, 0, 15, "Tier Status", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 15, tier_status, text_color=color.white, bgcolor=tier_status == "T1" ? color.green : tier_status == "T2" ? color.blue : tier_status == "T3" ? color.orange : color.red, text_size=size.small)
    
    table.cell(info_table, 0, 16, "P90 Confirmed 2-6AM", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 16, p90_confirmed_2_6am ? "YES ✓" : "NO ✗", text_color=color.white, bgcolor=p90_confirmed_2_6am ? color.green : color.red, text_size=size.small)
    
    table.cell(info_table, 0, 17, "Regime Status (9AM)", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 17, regime_status, text_color=color.white, bgcolor=regime_status == "CONFIRMED" ? color.green : regime_status == "CAUTION" ? color.orange : regime_status == "FAILED" ? color.red : color.gray, text_size=size.small)
    
    table.cell(info_table, 0, 18, "2 AM Target", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 18, asian_range_complete ? str.tostring(base_target_pips, "#") + " (±" + str.tostring(base_precision, "#.#") + ")" : "WAITING", text_color=color.white, bgcolor=asian_range_complete ? color.blue : color.gray, text_size=size.small)
    
    table.cell(info_table, 0, 19, "6 AM Target", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 19, checkpoint_6am_done ? str.tostring(adjusted_target_6am, "#") + " (±" + str.tostring(precision_6am, "#.#") + ")" : "WAITING", text_color=color.white, bgcolor=checkpoint_6am_done ? color.orange : color.gray, text_size=size.small)
    
    table.cell(info_table, 0, 20, "9 AM Target", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 20, checkpoint_9am_done ? str.tostring(final_target_9am, "#") + " (±" + str.tostring(precision_9am, "#.#") + ")" : "WAITING", text_color=color.white, bgcolor=checkpoint_9am_done ? color.green : color.gray, text_size=size.small)
    
    table.cell(info_table, 0, 21, "Accuracy Est.", text_color=color.white, bgcolor=color.gray, text_size=size.small)
    table.cell(info_table, 1, 21, accuracy_est, text_color=color.white, bgcolor=regime_status == "CONFIRMED" and p90_confirmed_2_6am ? color.green : color.gray, text_size=size.small)
...
# Write the Pine Script file
output_path = r"c:\Users\wifik\Desktop\projects\larger-lab\quant-lab\strategies\CEREBUS_P90_V5_PER_ASSET.pine"
with open(output_path, "w", encoding="utf-8") as f:
    f.write(pine_script)

print(f"✅ Generated: {output_path}")
print(f"   Assets: {len(ASSET_P90_CONFIGS)}")
print(f"   Size: {len(pine_script)} chars")
