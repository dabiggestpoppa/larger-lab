"""
P90 Cascade — Calibrated Multi-Pair Backtest
==============================================
1. Calibrates P90 thresholds per pair from historical data (90th percentile)
2. Runs P90 cascade backtest with calibrated thresholds
...
from p90_engine import P90Engine, P90Variant, P90Signal, Bar, TradeDirection, calibrate_p90
...
def run_p90_calibrated(symbol: str, bars: list, config: dict) -> dict:
    """Run P90 with calibrated thresholds."""
    # Step 1: Calibrate P90 thresholds
    p90_config = calibrate_p90(bars, est_hours=(3, 4, 5, 6, 7, 8, 9, 10, 11))

    # Step 2: Create engine with calibrated config
    engine = P90Engine(
    ...
    total = len(entries)
    if total == 0:
        return {"trades": 0, "p90_config": {k: round(v, 4) for k, v in p90_config.items()}}
    ...
    return {
        ...
        "p90_config": {k: round(v, 4) for k, v in p90_config.items()},
    }
...
def main():
    ...
    for symbol in all_pairs:
        ...
        result = run_p90_calibrated(symbol, bars, config)
        ...
        if result["trades"] > 0:
            ...
            print("  P90 config: %s" % result.get("p90_config", {}))
        else:
            print("  No trades generated")

    # Summary
    print("\n\n%s" % "=" * 90)
    print("  P90 CASCADE CALIBRATED — MULTI-PAIR BACKTEST SUMMARY")
    print("=" * 90)
    ...
    with open(report_dir / "p90_calibrated_all_pairs.json", "w") as f:
        json.dump(all_results, f, indent=2, default=str)
    print("\nResults saved to %s" % (report_dir / "p90_calibrated_all_pairs.json"))
