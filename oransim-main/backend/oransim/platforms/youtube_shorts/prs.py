"""Platform Recommender Surrogate (PRS) stub for YouTube Shorts.
...
to :class:`YouTubeShortsWorldModel` for structural predictions.
"""
...
V25_QUANTILE_PATH = Path("data/models/youtube_shorts_world_model_v25_quantile.pkl")
V2_PCA_PATH = Path("data/models/youtube_shorts_world_model_v2_pca.pkl")


class YouTubeShortsPRS:
    """YT Shorts PRS stub — same interface as TikTokPRS / XHSPRS."""
    ...
    def info(self) -> dict:
        if not self.is_ready():
            return {
                "loaded": False,
                "reason": (
                    "YT Shorts PRS pkl not shipped in OSS build. "
                    "Fall through to YouTubeShortsWorldModel for structural predictions."
                ),
            }
        return {
            "loaded": True,
    ...
    def predict(
        ...
        raise NotImplementedError(
            "YouTubeShortsPRS pkl loaded but predict() not wired. "
            "Port XHSPRS.predict feature-build here when shipping."
        )


PRS = YouTubeShortsPRS()
