...
One authoritative RUN_ID per execution. One final evidence directory.
Correct test classification. Fail-closed meta-test evidence.
Comprehensive final gate. Shared local and CI implementation.
...
def make_run_id():
    """Generate one authoritative RUN_ID per execution."""
...
class Validator:
    ...
    def check_source_identity(self):
        ...
        if expected_owner != "dabiggestpoppa":
            errors.append(
                f"REPOSITORY_OWNER: expected dabiggestpoppa, got {expected_owner}"
            )
        if expected_name != "larger-lab":
            errors.append(
                f"REPOSITORY_NAME: expected larger-lab, got {expected_name}"
            )
        if expected_full != "dabiggestpoppa/larger-lab":
            errors.append(
                f"REPOSITORY: expected dabiggestpoppa/larger-lab, got {expected_full}"
            )
        ...
        if remote_normalized not in accepted_normalized and remote_url not in accepted:
            errors.append(
                f"REMOTE: '{remote_url}' not in accepted origins {accepted}"
            )
        ...
        if expected_branch and actual_branch and actual_branch != expected_branch:
            errors.append(
                f"BRANCH: expected '{expected_branch}', got '{actual_branch}'"
            )

        actual_commit = git.get("commit", "")
        expected_commit = self.target_commit or contract.get(
            "expected_implementation_commit_source", ""
        )
        if expected_commit and expected_commit.strip() and actual_commit != expected_commit:
            errors.append(
                f"COMMIT: expected '{expected_commit[:12]}', got '{actual_commit[:12]}'"
            )
        ...
        if expected_tree and expected_tree.strip() and actual_tree != expected_tree:
            errors.append(
                f"TREE: expected '{expected_tree[:12]}', got '{actual_tree[:12]}'"
            )
        ...
        if expected_root and actual_root_rel != expected_root:
            errors.append(
                f"PROJECT_ROOT: expected '{expected_root}', got '{actual_root_rel}'"
            )
        ...
        # GitHub Actions environment (CI context)
        ...
        if gha_repo:
            if gha_repo != "dabiggestpoppa/larger-lab":
                errors.append(
                    f"GITHUB_REPOSITORY: expected dabiggestpoppa/larger-lab, got {gha_repo}"
                )
            if gha_sha and actual_commit and gha_sha != actual_commit:
                errors.append(
                    f"GITHUB_SHA: {gha_sha[:12]} does not match HEAD {actual_commit[:12]}"
                )
            if gha_ref:
                expected_branch_ci = self._get_resolved_branch(git)
                if expected_branch_ci and gha_ref != expected_branch_ci:
                    errors.append(
                        f"GITHUB_REF_NAME: expected '{expected_branch_ci}', got '{gha_ref}'"
                    )

        if self.authoritative:
            if not self.target_commit or not self.target_commit.strip():
                errors.append(
                    "AUTHORITATIVE: --target-commit is mandatory in authoritative mode"
                )
            if not self.target_tree or not self.target_tree.strip():
                errors.append(
                    "AUTHORITATIVE: --target-tree is mandatory in authoritative mode"
                )
            if not self.target_branch or not self.target_branch.strip():
                errors.append(
                    "AUTHORITATIVE: --target-branch is mandatory in authoritative mode"
                )
            if (
                actual_commit
                and self.target_commit
                and actual_commit != self.target_commit
            ):
                errors.append(
                    f"AUTHORITATIVE: HEAD {actual_commit[:12]} != target commit {self.target_commit[:12]}"
                )
            if actual_tree and self.target_tree and actual_tree != self.target_tree:
                errors.append(
                    f"AUTHORITATIVE: tree {actual_tree[:12]} != target tree {self.target_tree[:12]}"
                )
            if gha_repo:
                if gha_sha and actual_commit and gha_sha != actual_commit:
                    errors.append(
                        f"AUTHORITATIVE: GITHUB_SHA {gha_sha[:12]} != HEAD {actual_commit[:12]}"
                    )
                if gha_ref and self.target_branch and gha_ref != self.target_branch:
                    errors.append(
                        f"AUTHORITATIVE: GITHUB_REF_NAME '{gha_ref}' != '{self.target_branch}'"
                    )
                if gha_repo != "dabiggestpoppa/larger-lab":
                    errors.append(
                        f"AUTHORITATIVE: GITHUB_REPOSITORY '{gha_repo}' != 'dabiggestpoppa/larger-lab'"
                    )
            if expected_base and expected_base.strip() and actual_commit:
                if actual_commit != expected_base:
                    r = subprocess.run(
                    ...
                    if r.returncode != 0:
                        errors.append(
                            f"AUTHORITATIVE: HEAD is not descendant of base {expected_base[:12]}"
                        )
    ...
    def check_yaml_parsing(self):
        yaml_files = self._find_files(BASE_DIR, "*.yml") + self._find_files(
            BASE_DIR, "*.yaml"
        )
        yaml_files = [
            f for f in yaml_files if ".git" not in str(f) and "node_modules" not in str(f)
        ]
    ...
    def check_schema_validity(self):
        ...
        for s in schemas:
            try:
                ...
                self.add(
                    "SCHEMA-VALID",
                    "JSON Schemas valid Draft 2020-12",
                    True,
                    "BLOCKED",
        ...
        if valid_dir.exists():
            for fixture_file in sorted(valid_dir.glob("*.json")):
                ...
                for name, sp in schemas.items():
                    if schema_name.startswith(name) or name.startswith(
                        schema_name.split(".")[0]
                    ):
                        schema_path = sp
                        break
                ...
                try:
                    ...
                    failed += 1
                    details.append(
                        f"VALID fixture {fixture_file.name} rejected: {e.message[:100]}"
                    )
                except Exception as e:
                    failed += 1
        ...
        if invalid_dir.exists():
            for fixture_file in sorted(invalid_dir.glob("*.json")):
                ...
                try:
                    ...
                    failed += 1
                    details.append(
                        f"INVALID fixture {fixture_file.name} was accepted"
                    )
    ...
    def check_no_latest_tags(self):
        compose_files = self._find_files(COMPOSE_DIR, "*.yml") + self._find_files(
            COMPOSE_DIR, "*.yaml"
        )
        violations = []
    ...
    def check_digest_proof(self):
        ...
        if not evidence_path.exists():
            self.add(
                "DIGEST-PROOF",
                "Image digests match registry evidence",
                True,
                "BLOCKED",
        ...
        unverified = [
            name for name in evidence_digests if name not in verified_digests
        ]
        if violations:
            self.add(
                "DIGEST-PROOF",
            ...
            self.add(
                ...
                True,
                "PASS",
                f"{len(verified_digests)} verified",
                "All compose digests match",
            )
        ...
        for img in images:
            ...
            try:
                ...
                if r.returncode == 0 and (
                    "Manifests" in r.stdout
                    or "MediaType" in r.stdout
                    or "SchemaVersion" in r.stdout
                ):
                    resolved += 1
    ...
    def check_host_key_checking(self):
        ...
        if not cfg_path.exists():
            self.add(
                "HOST-KEY-CHECKING",
                "Ansible host_key_checking is True",
                True,
                "BLOCKED",
    ...
    def check_no_published_ports(self):
        ...
        for i, line in enumerate(content.split("\n"), 1):
            ...
            if in_services and "ports:" in line and not line.lstrip().startswith("#"):
                violations.append(
                    f"Line {i} in {current_service}: {line.strip()}"
                )
        if violations:
            self.add(
                "NO-DB-PORTS",
            ...
            for f in files:
                ...
                for i, line in enumerate(self._read_file(f).split("\n"), 1):
                    ...
                    if any(
                        ...
                        for kw in ["PLACEHOLDER", "CHANGE_ME", "REPLACE", "EXAMPLE"]
                    ):
                        continue
                    if re.search(pattern, stripped, re.IGNORECASE):
                        violations.append(
                            f"{f.relative_to(BASE_DIR)}:{i}: {desc}"
                        )
                        break
        if violations:
            self.add(
    ...
    def check_health_checks(self):
        ...
        if not foundation.exists():
            self.add(
                "HEALTH-CHECKS",
                "Health checks on foundation services",
                True,
                "BLOCKED",
                "compose.foundation.yml not found",
                "",
    ...
    def check_cost_thresholds(self):
        ...
        if not cost_file.exists():
            self.add(
                "COST-THRESHOLDS",
                "Cost thresholds match ratification",
                True,
                "FAIL",
    ...
    def check_worker_deny(self):
        ...
        if not policy.exists():
            self.add(
                "WORKER-DENY",
                "Workers denied DB/Redis/SSH/Docker",
                True,
                "FAIL",
        ...
        if deny_count >= 12:
            self.add(
                ...
                True,
                "PASS",
                f"{deny_count} DENY rules",
                f"{deny_count} denial rules",
            )
        else:
            self.add(
                "WORKER-DENY",
    ...
    def check_worker_no_db_access(self):
        ...
        if not policy.exists():
            self.add(
                "WORKER-NO-DB",
                "Workers denied direct DB access",
                True,
                "FAIL",
                "network-access.yml not found",
                "",
    ...
    def check_evidence_structure(self):
        ...
        if has_dir and has_templates:
            self.add(
                "EVIDENCE-DIR",
                "Evidence directory with templates",
                True,
                "PASS",
            ...
            self.add(
                ...
                True,
                "FAIL",
                f"dir={has_dir} templates={has_templates}",
                "Missing",
            )
    ...
    def check_ansible_syntax(self):
        ...
        try:
            ...
            r = subprocess.run(
                args, capture_output=True, text=True, timeout=30, cwd=str(BASE_DIR)
            )
            if r.returncode == 0:
                self.add(
                    "ANSIBLE-SYNTAX",
    ...
    def check_shellcheck(self):
        scripts = [
            s
            for s in list((BASE_DIR / "scripts").glob("*"))
    ...
    def check_scaffold_scan(self):
        ...
        if violations:
            self.add(
                "SCAFFOLD-SCAN",
                "No unresolved scaffolds in executables",
                True,
                "FAIL",
                f"{len(violations)} violations",
                "\n".join(violations),
    ...
    def check_totals_consistency(self, totals):
        expected_total = (
            totals["PASS"] + totals["FAIL"] + totals["BLOCKED"] + totals["SKIPPED"]
        )
        actual_total = totals["total"]
        if actual_total == 0:
            self.add(
                "TOTALS-CONSIST",
                "Result totals self-consistent",
                True,
                "FAIL",
                "0 results",
                "No checks executed",
    ...
    def check_fail_closed(self):
        ...
        Negative tests: complete mutation lifecycle required:
          baseline_result=PASS, baseline_exit=0,
          mutation_result=FAIL, mutation_exit!=0,
          post_restore_result=PASS, post_restore_exit=0,
          original_sha256 nonempty, restored_sha256 nonempty, hashes equal.

        Meta tests: must prove invalid fixture was rejected:
          fixture_type present, invalid_condition present,
          expected_rejection present, observed_rejection present, rejection_exit nonzero.
        """
        adv_path = self._evidence_dir() / "adversarial-results.json"
        if not adv_path.exists():
            self.add(
                "FAIL-CLOSED",
                "Adversarial mutations detected and rejected",
                True,
                "BLOCKED",
                "adversarial-results.json not found",
                "Run adversarial-tests.sh first",
            )
            return
        try:
        ...
        if adv.get("schema_version", "") != VERSION:
            errors.append(
                f"SCHEMA_VERSION: {adv.get('schema_version', '')} (expected {VERSION})"
            )
        if adv.get("validator_version", "") != VERSION:
            errors.append(
                f"VALIDATOR_VERSION: {adv.get('validator_version', '')} (expected {VERSION})"
            )

        # Validate RUN_ID is present and consistent
        adv_run_id = adv.get("run_id", "")
        if not adv_run_id:
            errors.append("RUN_ID: adversarial results have no run_id")
        elif adv_run_id != self.run_uid:
            errors.append(
                f"RUN_ID: adversarial run_id={adv_run_id} != validator run_id={self.run_uid}"
            )
        ...
        # Validate negative_tests: complete mutation lifecycle
        for t in negative_tests:
            ...
            if mr != "FAIL":
                errors.append(
                    f"{tid}: mutation_result={mr} (must be exactly FAIL)"
                )
            if me == 0 and mr not in ("N/A", ""):
                errors.append(
                    f"{tid}: mutation_exit=0 (must be nonzero)"
                )
            if pr != "PASS":
                errors.append(
                    f"{tid}: post_restore_result={pr} (must be PASS)"
                )
            if pe != 0:
                errors.append(f"{tid}: post_restore_exit={pe} (must be 0)")
            # Hash requirements: both nonempty and equal
            ...
            if orig and rest and orig != "N/A" and rest != "N/A" and orig != rest:
                errors.append(
                    f"{tid}: restored_hash {rest[:16]} != baseline_hash {orig[:16]}"
                )

        # Validate meta_tests: strict rejection evidence required
        for t in meta_tests:
            tid = t.get("test_id", "?")
            # Must have all rejection evidence fields
            for field in [
                "fixture_type",
                "invalid_condition",
                "expected_rejection",
                ...
                if not val and val != 0:
                    errors.append(
                        f"{tid}: meta_test missing required field {field}"
                    )
            # observed_rejection must prove rejection occurred
            obs = t.get("observed_rejection", "")
            if obs and obs not in ("FAIL", "BLOCKED"):
                errors.append(
                    f"{tid}: observed_rejection={obs} (must be FAIL or BLOCKED to prove rejection)"
                )
            # rejection_exit must be nonzero
            re_exit = t.get("rejection_exit", 0)
            if re_exit == 0:
                errors.append(
                    f"{tid}: rejection_exit=0 (must be nonzero to prove rejection)"
                )
            # Result should be PASS only when rejection is proven
            result = t.get("result", "")
            if result == "PASS" and (not obs or obs not in ("FAIL", "BLOCKED")):
                errors.append(
                    f"{tid}: result=PASS but no rejection evidence"
                )
    ...
    def check_meta_test_evidence(self):
        """Specifically validate meta-test rejection evidence.

        Each meta test must demonstrate:
        - fixture_type identifies what kind of invalid input was tested
        - invalid_condition describes the specific defect
        - expected_rejection names the expected check result (FAIL or BLOCKED)
        - observed_rejection is the actual result proving rejection
        - rejection_exit is nonzero proving the process rejected it
        """
        adv_path = self._evidence_dir() / "adversarial-results.json"
        if not adv_path.exists():
            self.add(
                "META-TEST-EVIDENCE",
                "Meta-test rejection evidence complete",
                True,
                "BLOCKED",
                "adversarial-results.json not found",
                "",
        ...
        for t in meta_tests:
            ...
            # observed must match expected
            if expected_rejection and observed_rejection:
                if expected_rejection not in ("FAIL", "BLOCKED"):
                    errors.append(
                        f"{tid}: expected_rejection={expected_rejection} (must be FAIL or BLOCKED)"
                    )
                if observed_rejection not in ("FAIL", "BLOCKED"):
                    errors.append(
                        f"{tid}: observed_rejection={observed_rejection} (must be FAIL or BLOCKED)"
                    )
    ...
    def check_run_id_consistency(self):
        ...
        # Check static-validation-results.json
        svr_path = ev_dir / "static-validation-results.json"
        if svr_path.exists():
            try:
                with open(svr_path) as f:
                    svr = json.load(f)
                svr_rid = svr.get("run_id", "")
                if not svr_rid:
                    errors.append("static-validation-results.json: no run_id")
                elif svr_rid != self.run_uid:
                    errors.append(
                        f"static-validation-results.json: run_id={svr_rid} != {self.run_uid}"
                    )
            except Exception:
                errors.append("static-validation-results.json: cannot parse")

        # Check adversarial-results.json
        adv_path = ev_dir / "adversarial-results.json"
        if adv_path.exists():
            try:
                with open(adv_path) as f:
                    adv = json.load(f)
                adv_rid = adv.get("run_id", "")
                if not adv_rid:
                    errors.append("adversarial-results.json: no run_id")
                elif adv_rid != self.run_uid:
                    errors.append(
                        f"adversarial-results.json: run_id={adv_rid} != {self.run_uid}"
                    )
            except Exception:
                errors.append("adversarial-results.json: cannot parse")

        # Check stage-status.json
        ss_path = ev_dir / "stage-status.json"
        if ss_path.exists():
            try:
                with open(ss_path) as f:
                    ss = json.load(f)
                ss_rid = ss.get("run_id", "")
                if not ss_rid:
                    errors.append("stage-status.json: no run_id")
                elif ss_rid != self.run_uid:
                    errors.append(
                        f"stage-status.json: run_id={ss_rid} != {self.run_uid}"
                    )
            except Exception:
                errors.append("stage-status.json: cannot parse")

        if errors:
            self.add(
                "RUN-ID-CONSISTENCY",
                "One RUN_ID consistent across all evidence",
                True,
                "FAIL",
                f"{len(errors)} mismatches",
                "\n".join(errors),
            ...
            self.add(
                ...
                True,
                "PASS",
                f"run_id={self.run_uid}",
                "All evidence artifacts use same RUN_ID",
            )
    ...
    def check_evidence_consistency(self):
        ...
        try:
            ...
            self.add(
                ...
                "Evidence identity matches checkpoint",
                True,
                "FAIL",
        ...
        if ev_commit and actual_commit and ev_commit != actual_commit:
            errors.append(
                f"COMMIT: evidence={ev_commit[:12]} actual={actual_commit[:12]}"
            )
        if ev_tree and actual_tree and ev_tree != actual_tree:
            errors.append(
                f"TREE: evidence={ev_tree[:12]} actual={actual_tree[:12]}"
            )
        ...
        if payload.get("validator_version", "") != VERSION:
            errors.append(
                f"VERSION: {payload.get('validator_version', '')} != {VERSION}"
            )
        ...
        ev_p = (
            ev_totals.get("PASS", 0)
            + ev_totals.get("FAIL", 0)
            + ev_totals.get("BLOCKED", 0)
            + ev_totals.get("SKIPPED", 0)
        )
        if ev_p != ev_total:
        ...
        if errors:
            ...
            self.add(
                ...
                "Evidence is self-consistent (atomic)",
            )
    ...
    def _build_results_payload(self, git_info):
        ...
        return {
            ...
            "tested_branch": self._get_resolved_branch(git_info)
            or git_info.get("branch", "unknown"),
    ...
    def write_evidence(self, git_info):
        ...
        self.write_summary_md(
            git_info, payload["totals"], payload["gate"], ev_dir
        )

        # Stage status
        self.write_stage_status(
            git_info, payload["totals"], payload["gate"], ev_dir
        )
    ...
    def write_summary_md(self, git_info, totals, gate, ev_dir=None):
        ...
        lines = [
            f"# B1-I1R3E Static Validation Summary",
            "",
        ...
        for r in self.results:
            lines.append(
                f"| `{r.check_id}` | {'yes' if r.mandatory else 'no'} | {r.result} | {r.evidence} |"
            )
    ...
    def write_stage_status(self, git_info, totals, gate, ev_dir=None):
        ...
        status = {
            ...
            "implementation_branch": self._get_resolved_branch(git_info)
            or git_info.get("branch", "unknown"),
            ...
            "cost_impact_usd": 0,
            "cloud_mutations": 0,
            "next_authorized_action": "Operator review of B1-I1R3E evidence",
        }
        for r in self.results:
            if r.result == "BLOCKED":
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=f"OCE Cloud Ground Validation Engine v{VERSION}"
    )
    ...
    parser.add_argument(
        "--only", type=str, help="Comma-separated check IDs to run"
    )
    ...
    print(f"\n{'='*50}")
    print(f"  B1-I1R3E Validation — {gate}")
    print(f"{'='*50}")
