...
| `OCE_POSTGRES_HOST` | http_api, spine | DB host | `postgres.host` | canonical | env | CANONICAL |
| `OCE_POSTGRES_PASSWORD_REF` | spine secret ref | DB credential | `postgres.password_ref` | canonical | env (reference only) | CANONICAL |
| `POSTGRES_PASSWORD` | local_secrets (init) | DB credential | — (feeds approved runtime store) | legacy init | env at `configure` time only, never at runtime | COMPATIBILITY_ADAPTER -> materialized into `.runtime/secrets.json` |
| `POSTGRES_DSN` | http_api, worker_loop (legacy) | DB connection | — (derived from secret ref) | legacy | NOT accepted at runtime entry | DEPRECATED_AND_REJECTED at runtime (derivation comes from the spine secret boundary) |
...
| `OCE_CAPITAL_AUTHORITY` | spine | capital | `capital.authority` | canonical | env | CANONICAL (none/approved; PO-owned) |
...
| `OCE_CP_URL` | oce_b3_worker | CP URL | — (must be loopback) | operational | env | OPERATIONAL |
...
`configure` secret materialization path. The durable runtime constructs its DSN
from the governed `postgres.password_ref` -> secret store -> ephemeral DSN
boundary (B4-R3R3/R4).

## Split-brain convergence (B4-R3R2+)
...
- Postgres DSN: single derivation path — secret reference -> approved local
  store -> in-memory DSN. `POSTGRES_DSN` at runtime entry is rejected.