...
# Both engines are in this directory, so pg-recovery's approved roots are
# exactly this engine's roots: the containment policy has one owner there
# (B4-CXR7U9R12/U9X2) and is reused, never re-stated, here.
_approved_roots = _PG._approved_roots
_validated_open_path = _PG._validated_open_path
_validated_read_text = _PG._validated_read_text


def load_protected_inventory(inventory_path, inventory_sha_path):
    """pg-recovery's tamper check, plus the verifier's own emptiness guard:
    an inventory with no tables could never prove a row."""
    inv = _PG._load_protected_inventory(inventory_path, inventory_sha_path)
