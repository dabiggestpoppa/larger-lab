...
python3 -m pytest "$TEST_FILE" "$CONTRACT_TEST" "$LIFECYCLE_TEST" "$GATE_TEST" "$COMPOSE_OUT_TEST" "$PORTABILITY_TEST" "$BACKUP_HARDEN_TEST" "$PATH_AUTHORITY_TEST" "$RECOVERY_AUTHORITY_TEST" "$RECEIPT_WRITE_TEST" "$TRANSITION_AUTHORITY_TEST" "$CROSS_STORE_TEST" "$TRANSACTION_CLOSURE_TEST" "$CLAIM_RACE_PROOF_TEST" "$CLAIM_RACE_INPROCESS_TEST" "$COMMIT_BOUNDARY_TEST" "$NEGATIVE_CONTROL_TEST" "$SHELL_COMMIT_LAW_TEST" "$CRASH_COHERENCE_TEST" "$R41R4_COMPLETION_TEST" -v --tb=short \
