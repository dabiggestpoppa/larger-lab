...
CLAIM_RACE_PROOF_TEST="$BASE_DIR/tests/test_b4_cxr7u9r40r1_operation_wide_claim.py"
CLAIM_RACE_INPROCESS_TEST="$BASE_DIR/tests/test_b4_cxr7u9r40r1_claim_race_inprocess.py"
...
R41R4_COMPLETION_TEST="$BASE_DIR/tests/test_b4_cxr7u9r41r4_completion.py"
R42_EXECUTION_AUTHORITY_RACE_TEST="$BASE_DIR/tests/test_b4_cxr7u9r42r2_execution_authority_race.py"
R43_LOCK_ROLLBACK_RESUME_TEST="$BASE_DIR/tests/test_b4_cxr7u9r43_lock_inode_rollback_resume.py"
R44_ATOMIC_CLAIM_PUBLICATION_TEST="$BASE_DIR/tests/test_b4_cxr7u9r44_atomic_claim_publication.py"
R44X2_CLAIM_TEMPORARY_LIVENESS_TEST="$BASE_DIR/tests/test_b4_cxr7u9r44x2_claim_temporary_liveness.py"
...
python3 -m pytest "$TEST_FILE" "$CONTRACT_TEST" "$LIFECYCLE_TEST" "$GATE_TEST" "$COMPOSE_OUT_TEST" "$PORTABILITY_TEST" "$BACKUP_HARDEN_TEST" "$PATH_AUTHORITY_TEST" "$RECOVERY_AUTHORITY_TEST" "$RECEIPT_WRITE_TEST" "$TRANSITION_AUTHORITY_TEST" "$CROSS_STORE_TEST" "$TRANSACTION_CLOSURE_TEST" "$CLAIM_RACE_PROOF_TEST" "$CLAIM_RACE_INPROCESS_TEST" "$COMMIT_BOUNDARY_TEST" "$NEGATIVE_CONTROL_TEST" "$SHELL_COMMIT_LAW_TEST" "$CRASH_COHERENCE_TEST" "$R41R4_COMPLETION_TEST" "$R42_EXECUTION_AUTHORITY_RACE_TEST" "$R43_LOCK_ROLLBACK_RESUME_TEST" "$R44_ATOMIC_CLAIM_PUBLICATION_TEST" "$R44X2_CLAIM_TEMPORARY_LIVENESS_TEST" -v --tb=short \
