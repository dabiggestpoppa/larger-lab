...
- tested SHA `19b6077f456b1df2a751d38385ae4d6ca3a7a54d`
...
- authoritative test command `cd stress-suite && PYTHONIOENCODING=utf-8 python -m pytest tests -q` -> **passed 1058**, and this count is not the artifact's: an ordinary run of this command executes the lag check that the artifact-producing run has to skip, so it passes 1057 (measured below) + 1 (the skipped case) = 1058 of 1058 collected
- artifact-producing command `cd stress-suite && PYTHONIOENCODING=utf-8 python -m pytest tests -q --junitxml=evidence/G8_TEST_RESULTS.xml` -> **collected 1058 / passed 1057 / skipped 1 / failed 0** (artifact `pytest` `9267e8b103e04279`, python `3.11.9`). These are the counts this package rests on, and they belong to the command that produced the artifact
