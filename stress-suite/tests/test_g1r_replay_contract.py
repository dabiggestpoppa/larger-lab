...
def _auth(**seeds) -> AuthorityState:
    """G2-P0: governed replay runs bind actors to AuthorityState."""
    auth = AuthorityState()
    for a, l in ({"PO": "PO", "SENTINEL": "GOVERNOR", **seeds}).items():
        auth.seed_level(a, l)
...
def test_replay_uses_supplied_lifecycle_contract():
    ...
    default = DeterministicReplay(lifecycle_table=V1, seed_records=_seed_lifecycle(), authority=_auth()).run(evs)
    assert default.trace[0]["allowed"] is False
    # custom table: TESTED->ACTIVE IS legal
    custom = DeterministicReplay(lifecycle_table=V2, seed_records=_seed_lifecycle(), authority=_auth()).run(evs)
    assert custom.trace[0]["allowed"] is True


def test_replay_custom_lifecycle_contract_changes_result():
    evs = _lifecycle_events()
    a = DeterministicReplay(lifecycle_table=V1, seed_records=_seed_lifecycle(), authority=_auth()).run(evs)
    b = DeterministicReplay(lifecycle_table=V2, seed_records=_seed_lifecycle(), authority=_auth()).run(evs)
...
def test_replay_same_custom_contract_is_deterministic():
    evs = _lifecycle_events()
    a = DeterministicReplay(lifecycle_table=V2, seed_records=_seed_lifecycle(), authority=_auth()).run(evs)
    b = DeterministicReplay(lifecycle_table=V2, seed_records=_seed_lifecycle(), authority=_auth()).run(evs)
    assert a.fingerprint == b.fingerprint
...
def test_replay_blank_contract_version_policy_is_explicit():
    ...
    evs = _lifecycle_events()
    res = DeterministicReplay(lifecycle_table=V1, seed_records=_seed_lifecycle(), authority=_auth()).run(evs)
