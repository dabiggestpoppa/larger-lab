...
TESTED_SHA = "3d3f8836b07432a37e7ab119b3c6848c64e442be"
...
#: the same command with the artifact destination the receipt cites. Run from the
#: package directory, so the artifact lands INSIDE the repository and a reviewer
#: can resolve the cited path (STRESS-G8RX6, R-G8-07 provenance gap).
ARTIFACT_COMMAND = (AUTHORITATIVE_TEST_COMMAND + " --junitxml="
                    + ARTIFACT_RELATIVE_PATH.split("stress-suite/", 1)[1])
...
def emit(test_evidence: TestEvidence) -> Dict[str, Any]:
    ...
    the caller declared, so the path the receipt cites can be resolved by a
    reviewer on any machine. An artifact at a machine-local temp path is refused
    rather than published, and the receipt cites the repo-relative location.
    """
    if not test_evidence.in_tree:
        raise UnverifiableTestEvidence(
            f"the bound test artifact {test_evidence.artifact_path!r} is not "
            "inside the declared evidence tree, so the receipt cannot cite a "
            "repo-relative location a reviewer could resolve; refusing emission")
...
def main(argv: Sequence[str]) -> Dict[str, Any]:
    ...
    There is deliberately no path that accepts a bare count, and the artifact
    must lie inside this repository at the declared evidence path."""
    ...
    if evidence.repo_relative_path != ARTIFACT_RELATIVE_PATH:
        raise UnverifiableTestEvidence(
            f"the authoritative build publishes {ARTIFACT_RELATIVE_PATH!r}; this "
            f"artifact is at {evidence.repo_relative_path!r}. Produce it with "
            f"`{ARTIFACT_COMMAND}` so the receipt always cites the same path")
    out = emit(evidence)
