...
def compile_mql5(filepath: str) -> tuple[bool, str]:
    ...
    if sys.platform == "win32":
        possible = [
            ...
            r"C:\Program Files\Ox Securities MetaTrader 5\MetaEditor64.exe",
        ]
        for p in possible:
            if os.path.exists(p):
                metaeditor_path = p
...
def mt5_get_market_data(
    ...
    **Returns:** Formatted price data (last 10 bars shown) and summary stats.
    ...
    try:
        ...
        import tempfile
        csv_path = os.path.join(tempfile.gettempdir(), f"{symbol}_{timeframe}.csv")
...
'''
    ...
    # Merge custom inputs with defaults - custom inputs override defaults
    if inputs:
        # Parse custom inputs to check which variables are defined
        import re
        # Match variable names from input declarations (handles both single and multiple spaces)
        custom_vars = set(re.findall(r'input\s+\w+\s+(\w+)\s*[=;]', inputs))
        # Add missing defaults
        missing_defaults = []
        for line in default_inputs.strip().split('\n'):
            # Match variable name from default input lines
            var_match = re.search(r'input\s+\w+\s+(\w+)\s*[=;]', line)
            if var_match and var_match.group(1) not in custom_vars:
                missing_defaults.append(line)
        final_inputs = inputs + '\n' + '\n'.join(missing_defaults)
    else:
        final_inputs = default_inputs
