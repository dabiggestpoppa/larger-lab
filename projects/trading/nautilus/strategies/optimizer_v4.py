#!/usr/bin/env python3
"""
Quant Lab Optimizer v4 - SL/TP Balance Fixes + Alpha Combination Layer
=======================================================================
V3 bug fixes plus V4 targeted SL/TP rebalancing for losing strategies.
Two_Plays: SL 1.5x body, TP ARx0.35 | Dual_Engine: SL 1.5x body, TP ARx0.35
Alpha_Combo: threshold 0.20, SL 1.5x body | BSC: SL 5p buffer, TP 0.8x impulse
Fractal_Res: TP 1.0x impulse (down from 1.44x) | Based on RohOnChain IR framework.

V3 Bug Fixes (all preserved):
...
5. Two_Plays: Fixed Base 80 SL tightness + T3 Model 2 logic
...
V4 SL/TP Rebalancing (based on v3 exit pattern analysis):
- Two_Plays: SL 1.0x->1.5x body, TP ARx0.50->ARx0.35
- Dual_Engine anchor: SL 0.8x->1.5x body, TP ARx0.50->ARx0.35
- Alpha_Combination: threshold 0.35->0.20, SL 0.8x->1.5x body
- Blind_Structural_Chain: SL buffer 2p->5p, TP 1.0x->0.8x impulse
- Fractal_Resolution: TP 1.44x->1.0x impulse
"""
import sys
import json
import time
...
def manage_trade_partial(post_df, entry_price, direction, sl, tp, hard_exit_est=17):
    """
    Trade management with partial profit taking.
    - 50% closed at TP1 (halfway to TP)
    - SL moved to entry + 2p (breakeven) after TP1 hit
    - Remaining 50% closed at TP (full target)
    Returns single trade dict with combined pnl.
    """
    if post_df.empty:
        return None
    
    tp1 = (entry_price + tp) / 2.0 if direction == 'LONG' else (entry_price + tp) / 2.0
    be_level = entry_price + to_price(2.0) * (1 if direction == 'LONG' else -1)
    half_closed = False
    ...
    for idx, row in post_df.iterrows():
        ...
        if row['est_h'] >= hard_exit_est:
            if not half_closed:
                ...
                pnl1 = to_pips(tp1 - entry_price) * (1 if direction == 'LONG' else -1)
                pnl2 = to_pips(c - entry_price) * (1 if direction == 'LONG' else -1)
                total = pnl1 * 0.5 + pnl2 * 0.5
                return {'pnl': total, 'result': 'W' if total > 0 else 'L',
                        'reason': 'hard_exit', 'exit_price': c, 'exit_time': idx}
        ...
        if direction == 'LONG':
            if not half_closed:
                ...
                if l <= be_level:
                    pnl1 = to_pips(tp1 - entry_price)
                    pnl2 = to_pips(be_level - entry_price)
                    total = pnl1 * 0.5 + pnl2 * 0.5
                    return {'pnl': total, 'result': 'W' if total > 0 else 'L',
                            'reason': 'tp_be', 'exit_price': be_level, 'exit_time': idx}
                if h >= tp:
                    pnl1 = to_pips(tp1 - entry_price)
                    pnl2 = to_pips(tp - entry_price)
                    total = pnl1 * 0.5 + pnl2 * 0.5
                    return {'pnl': total, 'result': 'W', 'reason': 'tp_full', 'exit_price': tp, 'exit_time': idx}
        else:
            if not half_closed:
                if h >= sl:
                    pnl = to_pips(entry_price - sl)
                ...
                if h >= be_level:
                    pnl1 = to_pips(entry_price - tp1)
                    pnl2 = to_pips(entry_price - be_level)
                    total = pnl1 * 0.5 + pnl2 * 0.5
                    return {'pnl': total, 'result': 'W' if total > 0 else 'L',
                            'reason': 'tp_be', 'exit_price': be_level, 'exit_time': idx}
                if l <= tp:
                    pnl1 = to_pips(entry_price - tp1)
                    pnl2 = to_pips(entry_price - tp)
                    total = pnl1 * 0.5 + pnl2 * 0.5
                    return {'pnl': total, 'result': 'W', 'reason': 'tp_full', 'exit_price': tp, 'exit_time': idx}
    ...
    for date in sorted(df['date'].unique()):
        ...
        if tier in ('T1', 'T2'):
            ...
            for idx, row in entry.iterrows():
                ...
                if row['close'] > ah:
                    direction = 'LONG'
                    close_dist = to_pips(row['close'] - ah)
                elif row['close'] < al:
                    direction = 'SHORT'
                    close_dist = to_pips(al - row['close'])
                else:
                    continue
                
                # Quality filter: close must be at least 2p outside Asian band
                # and AR must be in T1 range (tightest = best edge)
                if close_dist < 2.0 or ar > 20:
                    continue
...
def run_dual_engine(df):
    ...
    for date in sorted(df['date'].unique()):
        ...
        for idx, row in post_anchor_entry.iterrows():
            ...
            if 0.32 <= retrace_pct <= 0.50:
                ep = row['close']
                sl = ep - to_price(amp_body * 1.5) * (1 if amp_direction == 'LONG' else -1)
                tp = ep + to_price(ar * 0.35) * (1 if amp_direction == 'LONG' else -1)
...
def run_alpha_combination(df):
    ...
    for date in sorted(df['date'].unique()):
        ...
        # Quality filter: close must be at least 2p outside Asian band
        if direction == 'LONG':
            close_dist = to_pips(ep - ah)
        else:
            close_dist = to_pips(al - ep)
        if close_dist < 2.0 or ar > 20:
            continue
        
        sl = ep - to_price(p90_body * 1.5) * (1 if direction == 'LONG' else -1)
        base_tp = ar * (0.25 + 0.15 * min(composite, 1.0))
        tp = ep + to_price(base_tp) * (1 if direction == 'LONG' else -1)
...
def main():
    print("=" * 70)
    print("[TEST] QUANT LAB OPTIMIZER v4 - SL/TP Balance Fixes + Alpha Combination")
    print("=" * 70)
