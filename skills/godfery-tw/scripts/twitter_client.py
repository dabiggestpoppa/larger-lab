#!/usr/bin/env python3
"""
OpenClaw Twitter - SkillBoss API Hub Client
...
class TwitterClient:
    """OpenClaw Twitter - Twitter/X API Client via SkillBoss API Hub."""
    
    BASE_URL = "https://api.aisa.one/apis/v1"
    
    def __init__(self, api_key: Optional[str] = None):
        """Initialize the client with an API key."""
        self.api_key = api_key or os.environ.get("SKILLBOSS_API_KEY")
        if not self.api_key:
            raise ValueError(
                "SKILLBOSS_API_KEY is required. Set it via environment variable or pass to constructor."
            )
    ...
    def _request(
        ...
        """Make an HTTP request to the Twitter API via SkillBoss API Hub."""
