...
(async () => {
  try {
    // Check study values for DMR v4
    const sv = await mcpCall('data_get_study_values', { study_name: 'CEREBUS DMR v4' });
    console.log('Study values:', sv);

    // Check tables
    const tables = await mcpCall('data_get_pine_tables', { study_filter: 'DMR v4' });
    console.log('Tables:', tables);

    // Open strategy tester and screenshot
    ...
    const ss = await mcpCall('capture_screenshot', { region: 'full', format: 'path', filename: 'tv_v4_backtest.png' });
