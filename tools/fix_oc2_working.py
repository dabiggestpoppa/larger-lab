"""Fix OC2 config - use openrouter/owl-alpha which is properly registered."""
import json
...
# Remove any stale providers
cfg["models"]["providers"].pop("inclusionai", None)
cfg["models"]["providers"].pop("google", None)

# Make sure openrouter/owl-alpha is in the openrouter models list
...
# Remove inclusionai/ring-2.6-1t from openrouter models (it was never a real openrouter model)
or_models = [m for m in or_models if not m.get("id", "").startswith("inclusionai/")]
