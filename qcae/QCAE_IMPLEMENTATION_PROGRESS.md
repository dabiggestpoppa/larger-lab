...
**Residual, reported not fixed:** in the receipt-without-candidates shape the record carries both `FULLY_SATISFIED_INTERNAL` and `PARTIALLY_SATISFIED_INTERNAL` in `classifications` (verdict `FULLY_SATISFIED_INTERNAL`). The upstream cause is P1's `DEFINITION_WITHOUT_IMPLEMENTATION` category (`sqlite_knowledge_store.internal_first_findings`), which fires from the candidate inventory alone and ignores an active receipt, so P1 emits "defined but unimplemented" while its own receipt table proves an implementation. P3 carries every P1 finding through rather than dropping one, and this coexisting-pair pattern is already the established convention there (shape without any candidates is pinned by `test_definition_without_implementation_is_partial`). Suppressing it in P3 would hide a P1 finding; correcting it in P1 changes another phase's asserted category output. Left as a decision for whichever pass owns P1's retrieval classification.
