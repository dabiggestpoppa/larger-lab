...
**Found while clearing (reproduced over a real P1 registry, not fixed — it is a behavior change, and this pass changes only shape):** a capability with an **active receipt and no registered candidates** cannot produce an internal baseline at all. `CAPABILITY_ACTIVE` maps to `FULLY_SATISFIED_INTERNAL` (canon 2.6.1 table, `baseline.py:122`), but `_capability_granularity_coverage` (`baseline.py:493`) returns empty coverage whenever `candidate_refs` is empty, so `build()` assembles a record whose verdict claims full internal satisfaction while `missing_atoms` is the whole plan scope — and `validate()` then refuses it: `FULLY_SATISFIED_INTERNAL cannot leave uncovered atoms (canon 2.6.1)`. Four wiring shapes probed: receipt + no candidates → REFUSED; receipt + one candidate → OK (`covered=2`); receipt + candidates for both atoms → OK; no receipt + one candidate → OK (`PARTIALLY_SATISFIED_INTERNAL`). Every existing `CAPABILITY_ACTIVE` test fixture also passes `candidate_refs`, which is why the suite never sees it. Needs a decision first: either coverage must count a proven implementation (a receipt) as holding the capability's atoms, or `build()` must refuse *before* assembling, with an error that names the condition instead of an internal-consistency assertion.
