"""P2-R2-FREEZE — superseding governance-wiring repair manifest.

History-preserving chain (P1-R1/P2-R1 convention):
...
- ``P2-R2-freeze-manifest.json`` = THIS superseding manifest, recording the
  governance-wiring repair tranche (C01..T01), the prior manifests' commits
  + digests, the operator-review trigger, and fresh fail-closed test
  evidence captured from an actual full-suite run at the tested commit.
...
Run:  python qcae/implementation/tools/p2r2_freeze_manifest.py [expected_head_sha]
"""
...
#: Commits of the original P2 build (preserved history).
P2_ORIGINAL_COMMITS = [
...
#: The C07R structural repair tranche (P2-R1).
P2_R1_COMMITS = [
    "425ac5f6", "3b5aaed0", "bfead6f6", "acf31b6f", "cb1cede4",
    "6ea6dcef", "ac0dd3c2", "ea493b8e", "a8b20a6c",
]

#: The governance-wiring repair tranche (P2-R2, this manifest's subject).
P2_R2_COMMITS = [
    "30cebe27",  # I0: audit + repair plan recorded in the ledger
    "407b7de9",  # C01: StepAuthorityGate wired into execute_step; bypass removed
    "7f6bccb7",  # C02: approval -> execution round trip (exact-scope release)
    "e93f41dd",  # C03: recovery re-evaluation through the real gate
    "3a7783ea",  # C04: canonical JobSubmission + durable CLI submit
    "4b88cbd7",  # C05: private-boundary leaks removed + architecture guard
    "f1e02304",  # C06: authority x budget x secrets integration laws
    "8e8b9431",  # T01: governance adversarial qualification
]


def build_manifest(tested_commit: str, evidence: dict) -> dict:
    return {
        "phase": "P2-R2 — Governance-Wiring Repair (superseding)",
        "supersedes": {
            ...
            "relationship": (
                ...
                "P2-R1 repair manifest are preserved unchanged as historical truth"
            ),
        },
        "operator_review_trigger": (
            "Operator review of P2-R1 head a8b20a6c: governance components "
            "were built but not bound into the execution path — public "
            "authority_ok bypass, REQUIRE_APPROVAL not operational on the "
            "normal path, caller-modeled recovery re-evaluation, private "
            "engine boundary leaks, no durable CLI job submission; P3 NOT authorized"
        ),
        "canon_version": "0.1",
        ...
        "prior_freeze_refs": [
            ...
            P2_R1_FREEZE + " (historical, superseded by this manifest)",
        ],
        "repairs": {
            "c01_authority_gate_in_execution_path": {
                "defect": (
                    "execute_step exposed a public authority_ok=True caller "
                    "bypass; AuthorityProvider was never consulted on the "
                    "normal execution path"
                ),
                "repair": (
                    "StepAuthorityGate port + typed StepAuthorityVerdict binding "
                    "principal/action/resource/scope/requirement; engine evaluates "
                    "the CURRENT gate inside execute_step; no gate configured "
                    "fails closed; DENY -> POLICY_DENIED, REQUIRE_APPROVAL -> "
                    "durable request + WAITING_POLICY; verdict step/principal "
                    "replay guards; composition root wires "
                    "PolicyStepAuthorityGate + ApprovalRegistrySink with a "
                    "registered runtime worker principal; policy engine action "
                    "matcher honors prefix rules"
                ),
                "tests": "qcae/tests/unit/test_p2r2_c01_authority_gate.py",
            },
            "c02_approval_execution_round_trip": {
                "defect": (
                    "REQUIRE_APPROVAL blocked nothing durable: no request "
                    "artifact, no verified grant release path"
                ),
                "repair": (
                    "exact-bound AuthorityRequest with explicit 24h window; "
                    "service.release_granted_step verifies the GRANT against "
                    "the request (action/resource/scope/budget, not expired) "
                    "before engine release; single-use grant key; laundering "
                    "rejected at registry write AND re-checked service-side"
                ),
                "tests": "qcae/tests/unit/test_p2r2_c02_approval_round_trip.py",
            },
            "c03_recovery_authority_reevaluation": {
                "defect": "recovery re-evaluation was modeled by a caller flag",
                "repair": (
                    "resumed execution evaluates the live gate, so policy "
                    "changes during a crash window change resumed outcomes; "
                    "committed execution records and terminal steps untouched"
                ),
                "tests": "qcae/tests/unit/test_p2r2_c03_recovery_authority.py",
            },
            "c04_canonical_submission": {
                "defect": "no QcaeApp.job_submit; CLI could not durably submit",
                "repair": (
                    "versioned JobSubmission envelope validated canonically "
                    "before persistence (nothing persists on malformed input); "
                    "deterministic identity; atomic engine path; thin CLI "
                    "`job submit` durable across process close"
                ),
                "tests": "qcae/tests/unit/test_p2r2_c04_submission.py",
            },
            "c05_boundary_leaks_removed": {
                "defect": (
                    "QcaeApp used service._engine; RuntimeService.identity "
                    "inspected engine._workers"
                ),
                "repair": (
                    "public engine.registered_worker_types(); service "
                    "mark_running/require_identity/authority_decide surfaces; "
                    "TestInterfaceBoundary architecture guard (self-verified)"
                ),
                "tests": (
                    "qcae/tests/architecture/test_p0_dependency_guards.py::"
                    "TestInterfaceBoundary"
                ),
            },
            "c06_authority_budget_secrets_integration": {
                "defect": "cross-layer laws were neither stated nor proven",
                "repair": (
                    "authority cannot widen budget (approval_ref-only increases; "
                    "no engine widening path); action permission does not imply "
                    "secret permission; constrained grants stay constrained "
                    "across retries and are visible in the event trail"
                ),
                "tests": "qcae/tests/unit/test_p2r2_c06_integration.py",
            },
        },
        "persistence": {
        ...
        "vocabularies": {
            ...
            "approval_states": ["PENDING", "GRANTED", "DENIED", "EXPIRED"],
        ...
        "queue_lease_semantics": {
            "claim": (
                "atomic, eligibility-scoped inside the claim; guarded upsert; "
                "one active owner per step; empty eligibility creates no claim rows"
            ),
        ...
        "event_semantics": {
            ...
            "authority_payload": "STEP_STARTED carries the typed authority decision",
        },
        ...
        "cli_commands": [
            "job list", "job submit", "job status", "job events", "job run",
            "job resume", "job cancel", "approval list", "approval decide",
            "recover", "identity",
        ],
        ...
        "crash_recovery_evidence": {
            "tests": [
                ...
                "test_p2_c07r2_execution_semantics.py (crash windows A-G)",
                "test_p2r2_c03_recovery_authority.py",
            ],
            "result": (
                "completed steps never repeated; expired leases recovered; "
                "NON_REPLAY_SAFE ambiguity escalated; resumed execution "
                "re-evaluates the live gate (policy changes during crash "
                "windows change outcomes)"
            ),
        },
        ...
        "architecture_guard_evidence": {
            ...
            "interface_boundary": "TestInterfaceBoundary forbids private-state reach-ins (P2-R2-C05)",
...
def main(expected_head: str | None = None) -> int:
    ...
    out = REPO_ROOT / "qcae" / "implementation" / "P2-R2-freeze-manifest.json"
